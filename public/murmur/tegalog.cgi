...
# ================================================================== #
#  てがろぐ - Fumy Otegaru Memo Logger Ver 4.4.0      [tegalog.cgi]  #
# ================================================================== #
...
# CGI名の取得方法 ※以前は修正が必要だったサーバでも、まずは 2 のままで動作を試してみて下さい。※ログインや画面遷移がうまくいかない場合には、この値を変更して下さい。(0:プロトコルから/1:ディレクトリから/2:ファイルだけ/3:フルパス/5:自動/9:決め打ち) ※標準は2
my $howtogetpath = 2;
...
my %libdat = (
	...
	ogimagedefault => 'https://www.nishishi.com/cgi/tegalog/default-tegalog-ogimage.jpg'	# OGP共通画像の未指定時用画像URL (※注：このURLは管理画面の[設定]→[補助設定]で変更できます。SNSで一度でも表示した後は、SNS側でキャッシュされるため、変更内容は【リアルタイムには反映されない】点にご注意下さい！)
);
...
# -------------------- #
my $versionnum = '4.4.0';
my $reqfumycts = 2019000;	# 2.19.0 Required
...
my $cgi = new CGI;
my $cginame = &lookupcgipath($howtogetpath , 'tegalog.cgi');	# 注:(パラメータを排したアドレスを使いたい場合を除いて)この変数を直接は参照しない。代わりに &getCgiPath()を呼ぶ。
...
my %cp = (
	...
	mode	=> $cgi->param('mode') || 'view',		# view / edit / write / admin / imageup / licence / export / rss / xmlsitemap / sitemap / gallery / picts / random / getbackup / passcheck / around
...
my %setdat = (
	...
	utilityneighbor => 1,					# ユーティリティリンク：隣接一括表示リンクを表示(0:しない、1:する)
	utilityneinum => 3,						# ユーティリティリンク：隣接一括表示の件数(1～9)
	...
	allow2ndlineinsecret => 0,				# 鍵付き投稿でも2行目の表示を許可(0:しない、1:する)
	...
	sitemappageoutput => 1,					# サイトマップページモードを出力(1:する、0:しない)
	sitemappageentries => 60,				# サイトマップページモードで1ページに表示する投稿数
	sitemappagedatebar => 0,				# サイトマップページモードに日付境界バーを表示(0:しない/1:する)
	sitemappageyname => 'サイトマップ',		# サイトマップページモード機能の名称
	sitemappageskindir => 'skin-sitemap',	# サイトマップページモード機能用のスキン名
	sitemappagefixed => 0,					# サイトマップページモードに先頭固定設定を反映(0:しない/1:する)
	sitemapsituation => 1,					# サイトマップページモードで該当件数とページ番号を表示(1:する/0:しない)

	galleryoutput => 1,						# ギャラリーモード機能を(1:使う、0:使わない)
	galleryname => 'ギャラリー',			# ギャラリーモード機能の名称
	galleryskindir => 'skin-gallery',		# ギャラリーモード機能用のスキン名
	gallerydatebar => 0,					# ギャラリーモード表示時に日付境界バーを表示(0:しない/1:する)
	gallerysituation => 1,					# ギャラリーモードで該当件数とページ番号を表示(1:する/0:しない)
	galleryentries => 24,					# ギャラリーモードで1ページに表示する投稿数

	pictsmodeoutput => 1,					# 画像一覧モード機能を(1:使う、0:使わない)
	pictsmodename => '画像一覧',			# 画像一覧モード機能の名称
	pictsmodeskindir => 'skin-picts',		# 画像一覧モード機能用のスキン名
	pictsmodedatebar => 0,					# 画像一覧モード表示時に日付境界バーを表示(0:しない/1:する)
	pictsmodesituation => 1,				# 画像一覧モードで該当件数とページ番号を表示(1:する/0:しない)
	pictsmodenolisted => 1,					# 画像一覧モードで nolisted フラグ付き画像の扱い(1:下げる投稿扱い/0:表示しない)
	pictsmodeentries => 24,					# 画像一覧モードで1ページに表示する投稿数
	...
	urllabeledlink => 1,					# 角括弧でリンクラベルを作るリンクに(1:する/0:しない)
	urlparenthesislink => 1,				# 丸括弧で囲まれたURLをテキストリンクに(1:する/0:しない)
	...
	urlexpandinsta => 1,					# URLの直前に [Insta] ラベルがあればインスタ枠を埋め込み(1:する/0:しない)
	urlexpandcodepen => 0,					# URLの直前に [Insta] ラベルがあればCodePen枠を埋め込み(1:する/0:しない)
	...
	hashtagspace => 2,						# ハッシュタグ挿入時に半角スペースを自動追加する位置(0:入れない/1:前/2:後)
	...
	searchmarkup => 0,						# 検索時のハイライトに使う要素名
	imageslistup => 6,						# 投稿画像リストに掲載する個数
	imageslistthumbfirst => 1,				# 投稿画像リストでも、サムネイル画像があればそれを表示(1:する/0:しない)
	...
	msgnotags => 'ハッシュタグは見つかりませんでした。(または、まだ集計されていません。)',	# ハッシュタグがないか未集計の場合のメッセージ
	...
	showDecoBtnBonA => 1,	showDecoBtnBonQ => 1,	decoBtnLabelB => 'Ｂ',		decoBtnElmntB => 'b',		# 装飾ボタン[B]の表示(編集画面,QUICKPOST) 1:表示/0:非表示
	showDecoBtnConA => 1,	showDecoBtnConQ => 1,	decoBtnLabelC => '色',		decoBtnElmntC => 'span',	# 装飾ボタン[C]の表示(編集画面,QUICKPOST)
	showDecoBtnDonA => 1,	showDecoBtnDonQ => 1,	decoBtnLabelD => '消',		decoBtnElmntD => 'del',		# 装飾ボタン[D]の表示(編集画面,QUICKPOST)
	showDecoBtnEonA => 1,	showDecoBtnEonQ => 1,	decoBtnLabelE => '強',		decoBtnElmntE => 'em',		# 装飾ボタン[E]の表示(編集画面,QUICKPOST)
	showDecoBtnFonA => 0,	showDecoBtnFonQ => 0,	decoBtnLabelF => '○',		decoBtnElmntF => 'span',	# 装飾ボタン[F]の表示(編集画面,QUICKPOST)	※デフォルトでは非表示
	showDecoBtnHonA => 1,	showDecoBtnHonQ => 1,	decoBtnLabelH => '隠す',	decoBtnElmntH => 'span',	# 装飾ボタン[H]の表示(編集画面,QUICKPOST)	※指定範囲を隠す
	showDecoBtnIonA => 1,	showDecoBtnIonQ => 1,	decoBtnLabelI => 'Ｉ',		decoBtnElmntI => 'i',		# 装飾ボタン[I]の表示(編集画面,QUICKPOST)
	showDecoBtnLonA => 1,	showDecoBtnLonQ => 1,	decoBtnLabelL => '≡',									# 装飾ボタン[L]の表示(編集画面,QUICKPOST)
	showDecoBtnMonA => 1,	showDecoBtnMonQ => 1,	decoBtnLabelM => '背',		decoBtnElmntM => 'span',	# 装飾ボタン[M]の表示(編集画面,QUICKPOST)
	showDecoBtnQonA => 1,	showDecoBtnQonQ => 1,	decoBtnLabelQ => '”',		decoBtnElmntQ => 'q',		# 装飾ボタン[Q]の表示(編集画面,QUICKPOST)
	showDecoBtnRonA => 1,	showDecoBtnRonQ => 1,	decoBtnLabelR => 'ル',									# 装飾ボタン[R]の表示(編集画面,QUICKPOST)
	showDecoBtnSonA => 1,	showDecoBtnSonQ => 1,	decoBtnLabelS => '小',		decoBtnElmntS => 'small',	# 装飾ボタン[S]の表示(編集画面,QUICKPOST)
	showDecoBtnTonA => 1,	showDecoBtnTonQ => 1,	decoBtnLabelT => '極',		decoBtnElmntT => 'small',	# 装飾ボタン[T]の表示(編集画面,QUICKPOST)
	showDecoBtnUonA => 1,	showDecoBtnUonQ => 1,	decoBtnLabelU => 'Ｕ',		decoBtnElmntU => 'u',		# 装飾ボタン[U]の表示(編集画面,QUICKPOST)
	...
	operationtype => 0,						# 基本動作(0:一般公開(無限定表示)/1:ログイン者限定表示)
	guestidtype => 0,						# ゲストIDの権限範囲(0:通常/1:読み専)
	privoploginmsg => '',					# ログイン者限定表示時のログイン案内文言
	...
	outputsitemapxml => 0,					# XML-SITEMAPをsitemap.txtに静的出力(0:しない/1:する)
	outputindexhtml => 0,					# グランドホームをindex.htmlに静的出力(0:しない/1:する)
	loadeditcssjs => 0,						# 編集画面でedit.cssやedit.jsを読み込(0:まない/1:む)
	loadedithtm => 0,						# 編集画面でedit.htmを読み込(0:まない/1:む)
	allowhtmlondesc => 0,					# 概要文の入力欄でHTMLを許可(0:しない/1:する)
...
my %globalFlags = (
	...
	alreadyReverseXml => 0	# 既にデータ配列をひっくり返したかどうか
);

# 現状把握情報群
my %status = (
	skindir => './'		# いま適用されているスキン(プレビュー/簡易問わず)のディレクトリ		※デフォルトはカレントディレクトリ
);
...
my %subfile = (
	...
	editCSS	=> 'edit.css',		# 編集フォーム上書きCSSファイル名
	editJS	=> 'edit.js',		# 編集フォーム上書きJavaScriptファイル名
	editHTM	=> 'edit.htm',		# 編集フォーム上書きHTMLファイル名
	sitemapXml => 'sitemap.txt',	# 編集フォーム上書きHTMLファイル名
	verupTool => 'tegup.php'	# バージョンアップ用プログラム
);
...
# フルパスの設定 (※設定に依って変わるので、設定を読み込んだ後で実行する必要があるため、この位置で処理する)
...
if( $cp{'skindir'} ne '' ) {
	...
	if( $cp{'skindir'} =~ m/#(.+)#/ ) {
		...
		$status{'skindir'} = $cp{'skindir'};		# スキンDIRを保持しておく (指定内容に不備がある場合は上の overrideskins でエラー終了しているハズなのでここでは考慮しない)
	}
}
elsif( $setdat{'skindirectory'} ne '' ) {
	# 簡易本番適用の場合
	&overrideskins( $setdat{'skindirectory'} ,'');		# スキン上書き
	$status{'skindir'} = $setdat{'skindirectory'};		# スキンDIRを保持しておく (指定内容に不備がある場合は上の overrideskins でエラー終了しているハズなのでここでは考慮しない)
}
...
elsif(	$cp{'mode'} eq 'imageup'	) { &modeImageup(); }			# データXML不要
...
elsif(	$cp{'mode'} eq 'picts'		) { &modePicts(); }				# データXML不要
elsif(	$cp{'mode'} eq 'random'		) { &modeRandom(); }
elsif(	$cp{'mode'} eq 'getbackup'	) { &modeGetbackupfile(); }		# データXML不要
elsif(	$cp{'mode'} eq 'passcheck'	) { &modePasscheck(); }
elsif(	$cp{'mode'} =~ /^around(\d*)$/	) { &modeAround($1); }
...
# -----------
# CGI名を得る		引数1：取得方法(0=プロトコルから,1=ディレクトリから,2=ファイル名だけ,3=フルパス,4=環境から,5=自動,9=固定)、引数2：固定の場合のCGIファイル名
# -----------
sub lookupcgipath
{	
	my $howto = shift @_ || 0;
	...
	# デフォルト値だけは無駄な処理を経ずにすぐに返すようにする
	if( $howto == 2 ) { return $cgi->url(-relative, 1) || './'; }

	# 各パターンを一括取得しておく
	my @variations = ();
	$variations[0] = $cgi->url();
	$variations[1] = $cgi->url(-absolute, 1);
	$variations[2] = $cgi->url(-relative, 1) || './';	# .htaccessのDirectlyIndexを使ってファイル名を省略してアクセスされている状況では、空文字が入る点に注意
	$variations[3] = $cgi->url(-full, 1);				# 0と同じハズ
	$variations[4] = $0;

	# 自動と固定以外の場合
	if(( $howto >= 0 ) && ( $howto <= 4 )) {
		return $variations[$howto];
	}

	# 自動の場合
	if( $howto == 5 ) {
		# 0～4を試す
		foreach my $onevar (@variations) {
			my $tryname = &fcts::getafterlastdirsep( $onevar );
			if( &isNoSlashNoBackslash( $tryname ) ) {
				# 1文字以上の文字があり、スラッシュを含まず、バックスラッシュも含まない場合は、それを採用
				return $tryname;
			}
		}
		# だめなら固定値を返す(ために、ここではThrough)
	}

	# 試験結果の表示
	if( $howto == -1 ) {
		my $count = 0;
		my $res = '設定値 <code>$howtogetpath</code> の取得結果リスト：<br><br>';
		foreach my $onevar (@variations) {
			$res .= "$count : $onevar → <u>" . &fcts::getafterlastdirsep( $onevar ) . "</u><br>\n";
			$count++;
		}
		$res .= '9 : ' . $fixfn . "<br>\n";
		&errormsg( $res );
	...
	if( $fixfn ne '' ) {
		...
		&errormsg('CGI PATHを取得できませんでした。 $howtogetpath の値を -1 にして取得状況を確認してみて下さい。');
	}

	return 'ERR';		# ここは実行されないハズ
}
sub isNoSlashNoBackslash
{
	my $str = shift @_ || return 0;

	# 1文字以上の文字があり、スラッシュを含まず、バックスラッシュも含まない場合は 1 を返す
	if(
		( $str ne '' ) &&
		( index($str, '/') == -1 ) &&
		( index($str, '\\') == -1 )
...
sub denyattack
{
	...
	# チェック対象のパラメータ
	my $targets = $cp{'page'} . $cp{'mode'} . $cp{'order'} . $cp{'datelim'} . $cp{'cat'} . $cp{'postid'} . $cp{'posts'} . $cp{'userid'} . $cp{'skindir'};
	...
	print '<html><head><title>Illegal Parameter</title></head><body><h1>400 Bad Request</h1><p>この種のパラメータは受け付けられません。</p></body></html>';
	exit;
}

# -------------------------------------
# 高頻度アクセスBotによる高負荷防止(仮)
# -------------------------------------
sub highloadPrevention
{
	# UAにbotが含まれる場合のみ実行
	if( index($ENV{'HTTP_USER_AGENT'},'bot') >= 0 ) {

		# チェック対象のパラメータ
		my $targets = $cp{'datelim'} . $cp{'userid'} . $cp{'hasgtag'};
		if( $targets ne '' ) {
			# それらのパラメータに1文字でも指定があれば403エラーを返す
			my $cgi = CGI->new;
			print $cgi->header(-status => 403, -type => 'text/html; charset=UTF-8');
			print '<html><head><title>Forbidden</title></head><body><h1>403 Forbidden: No need to crawl</h1><p>《高負荷対策》パラメータの組み合わせで同じ本文が何度も取得されるのを防止するための、クローラー向けアクセス制限です。このエラーで不都合な状況があれば、てがろぐ作者までお知らせ下さい。</p><p>※もしあなたが人間の閲覧者なら、クローラーだと誤解されにくいUA名で再度アクセスして下さい。</p></body></html>';
			exit;
		}
...
{
	# ログイン者限定表示の場合の制限
	if( $setdat{'operationtype'} == 1 ) {
		&loginCheckOnPrivateOperation();
	}
	...
	# スキン内のIF文を解釈するため、SITUATION:CLASSを生成しておく (ページ番号は1として)
	my $situationcls = &fcts::trim( &makeLimitClasses( $totaldatnum, 1 ));
...
# 		$setdat{'allowlinebreak'} = 0;			# 改行をそのまま改行として表示しない
	...
	# 該当データを分解して表示用データを作成
	my @cliphtmls = &expandDataBySkin( 0, $usebuiltinskin, '', $situationcls, @applydata );
...
sub modeRss
{
	...
	$setdat{'urlexpandcodepen'} = 0;	# CodePenを埋め込まない
...
sub modeXmlSitemap
{
	...
	# XML-SITEMAPを静的出力している場合
	if( $setdat{'outputsitemapxml'} == 1 ) {
		if(( -f $subfile{'sitemapXml'} ) && ( &islimited() == 0 )) {
			# ファイルが既に生成されていて、何の表示条件も限定されていないなら、生成済みXMLファイルへリダイレクトする
			my $xmlpath = &fcts::joinwithslash( $cgifulldir, $subfile{'sitemapXml'} );
			print "Location: $xmlpath\n\n\n";
			exit;
		}
		# ファイルがまだ生成されていないか、表示条件が限定されている場合は、動的出力する。
	}

	&outputXmlSitemap();
}

sub outputXmlSitemap
{
	my $outputType = shift @_ || 0;
	...
	$setdat{'separatepoint'} = 0;		# 日付境界バーは出力しない
	$setdat{'onepostpageutilitybox'} = 0;	# ユーティリティリンク枠を出力しない
	$cp{'postid'} = 0;					# 投稿IDを限定しない
	$cp{'search'} = '-$ps=lock';		# 鍵付き投稿は除外する

	# 出力に使用するXML-SITEMAPスキン
	my $xmlsitemapskinname = 'builtinskin-xmlsitemap';

	# 指定のXML-SITEMAP用スキンでVIEW MODEを実行
	&modeView($xmlsitemapskinname, $outputType, $outputName);
}
...
sub modeGallery
{
	...
	if( $setdat{'galleryoutput'} == 0 ) {
		# ギャラリーを出力しない場合の画面
		&showadminpage('NO GALLERY MODE','','<p>このCGIは、ギャラリーモードを使わない設定で動作しています。</p><p>ギャラリーモードを表示するためには、管理画面から設定を変更して下さい。</p>','CA');
		exit;
	}
...
{
	# 画像一覧モードを使うかどうか
	if( $setdat{'pictsmodeoutput'} == 0 ) {
		# 画像一覧を出力しない場合の画面
		&showadminpage('NO PICTS MODE','','<p>このCGIは、画像一覧モードを使わない設定で動作しています。</p><p>画像一覧モードを表示するためには、管理画面から設定を変更して下さい。</p>','CA');
		exit;
	}
	...
	if( $rescode <= 0 ) {
		# 読めなかったら
		&showadminpage('NO IMAGE INDEX (for PICTS MODE)','','<p>画像インデックスファイルが読めませんでした。</p><p>画像保存用ディレクトリが存在しないか、画像インデックスの生成に失敗したか、または読み取りに失敗しました。管理画面内の「画像の管理」を表示させると問題解決に役立つかもしれません。</p>','CA');
		exit;
	}

	# 画像インデックスのファイル情報から表示用XMLデータを生成して、グローバル変数の @xmldata の中身に上書きする
	my @tempxml = ();
	my $count = scalar @imgindex;	# 配列内の個数
	...
	foreach my $onerec (@imgindex) {
		...
		# 日付が 0001/01/01 00:00:00 の場合は(念のために保存された)存在しない画像の情報なのでスキップする
		if( $date eq '0001/01/01 00:00:00' ) {
			next;
		}

		# 調整：投稿者の記録がない場合はダミーIDを当てはめる
		if( $user eq '' ) {
			$user = 'Unknown';
		}

		# 調整：フラグに nolisted が含まれていれば
		my $newflag = '';
		if( $flag =~ m/nolisted/ ) {
			if( $setdat{'pictsmodenolisted'} == 0 ) {
				# 一切表示しない設定の場合はスキップする
				next;
			}
			else {
				# 下げる投稿として扱う（フラグ rear をセットする）
				$newflag = 'rear';
			}
		}
		...
		# 調整：ファイルサイズを単位付きに変換
		my $fsize = &fcts::byteswithunit( $bytes );

		# 調整：タイトルには、キャプションがなければファイル名を充てる
		my $title = $cap;
		if( $title eq '' ) {
			$title = $file;
		}

		# 画像一覧用の、てがろぐデータXML用の投稿本文を用意する。
		my $onecomment = 
			$title . ' <br />' . 					# 1行目:タイトル(キャプションorファイル名)
			'[PICT:FIG:' . $file . '] <br />' . 	# 2行目:FIG付き画像
			$fsize . ' <br />' . 					# 3行目:ファイルサイズ
			$showflag . '<br />' .					# 4行目:フラグ(※将来的にフラグ以外のメタデータも出力する場合はここに追加したい)
			$file; 									# 5行目:ファイル名(※前後に余分な文字列を出力せず、ファイル名だけを出力する)
		# てがろぐデータXMLのlog要素を生成する
		# 書式： <log><date>日付</date><id>連番</id><user>投稿者</user><flag>変換後フラグ</flag><comment>上記で用意した本文文字列</comment></log>
		my $onelog = '<log><date>' . $date . '</date><id>' . $count . '</id><user>' . $user . '</user><flag>' . $newflag . '</flag><comment>' . $onecomment . '</comment></log>';
	...
	# てがろぐXMLデータを上書きする(作っておいた仮配列に置き換える)
	@xmldata = @tempxml;

	# 画像一覧用の強制上書き設定
	$setdat{'entryperpage'} = $setdat{'pictsmodeentries'};	# 出力数
	$setdat{'imagefullpath'} = 1;	# 画像は絶対URLで出力
	$setdat{'insertalttext'} = 0;	# 画像出力の省略時に「(画像省略)」とは出力しない
	$setdat{'outputlinkkeepskin'} = 0;		# 本文中のリンクで一時適用中のスキンを維持しない (skinパラメータを併用する場合のため)
	$setdat{'onepostpageutilitybox'} = 0;	# 画像単独表示ページに、ユーティリティ枠を表示しない
	$setdat{'searchhighlight'} = 0;	# 検索時に検索語をハイライト表示しない (ファイル名を強調されると崩れるため)

	# 単独表示時ならサムネイルを無視する
	if( $cp{'postid'} > 0 ) {
		$setdat{'imagethumbnailfirst'} = 0;	# サムネイル画像があってもそれは表示しない
	}

	# 日付境界バーの設定を上書き
	if( $setdat{'pictsmodedatebar'} == 0 ) {
		# 使わない場合
		$setdat{'separatepoint'} = 0;
	}
	$setdat{'separatebaroutput'} = 0;		# (どちらにしても)HTML出力リンクは使わない

	# 画像一覧モード用のスキンを指定（※スキンがパラメータで別途指定されていない場合のみ：スキンが指定されているなら既にそれを読む準備は整っている）
	if( $cp{'skindir'} eq '' ) {
		my $pictsmodeskin = '';
		$pictsmodeskin = &fcts::safetyfilename( $setdat{'pictsmodeskindir'} ,0 );	# 画像一覧スキン用ディレクトリの前処理(安全化)
		if( $pictsmodeskin eq '' ) {
			$pictsmodeskin = 'skin-picts';		# 空白ならとりあえずデフォルト値を試す
		}

		# 指定のスキンでVIEW MODEを実行させる
		&overrideskins($pictsmodeskin,'画像一覧');		# 画像一覧用スキンで上書き (存在確認もこの関数でする)
		$setdat{'skindirectory'} = "$pictsmodeskin";
	}
...
sub modeRandom
{
	# (限定条件がある場合はそれを反映させた)表示対象データだけを抜き出す (下書きは強制排除、下げる投稿は設定次第)
	my @applydata = &extractApplyData();

	# データが1件以上ある場合は、ランダムに投稿IDを決める
	if( $#applydata >= 0 ) {
		my $rnum = int(rand( $#applydata + 1 ));				# 投稿データの中から1レコードをランダムに選ぶ
		my $rid  = &fcts::getcontent($applydata[$rnum],'id');	# その投稿番号を得る
		$cp{'postid'} = $rid;		# 表示用にID番号パラメータを上書き
	}

	# VIEW MODEを実行 (データが1件以下の場合はランダムではない通常の表示＝「1件も見つからない」旨の表示が出るはず。)
	&modeView('');
}

# ===========================
# ★AROUND MODE					引数：前後何件ずつを抽出するか
# ===========================	使用方法：?mode=around数&postid=123		HashtagやCatとの共存は可能。searchの併用は不可。
sub modeAround
{
	my $around = shift @_ || 0;		# 引数1：前後何件ずつを抽出するか

	if( $around == 0 ) {
		# 値が省略されていれば設定値を採用
		$around = $setdat{'utilityneinum'};
	}

	# 件数の確認
	if( $around < 1 || $around >= 10 ) {
		# 1未満か10以上なら
		&illegalparam('前後の表示件数は 1～9 の範囲で指定して下さい。'); exit;
	}

	# 総データ数の半分以上を指定しないようにする
	my $halfsize = int(($#xmldata + 1) / 2);
	if( $around >= $halfsize ) {
		# もし総データ数の半数以上が指定されていたら「半数-1」に強制修正する
		$around = $halfsize - 1;
	}

	# ページネーションを不要にする
	$setdat{'entryperpage'} = $around * 2 + 1;	# 1ページあたりの表示数を総数と一致させる
	$setdat{'eppoverride'} = 0;					# スキン側の表示数強制指定を無効化する

	# 中心の投稿番号を得る
	my $postid = $cp{'postid'};
	$cp{'postid'} = 0;				# postidパラメータ自体はリセットしておく
	if( $postid == 0 ) { &illegalparam('中心に表示する投稿ID（投稿番号）を指定して下さい。AROUNDモードでは、postidパラメータを省略できません。'); exit; }

	# (限定条件がある場合はそれを反映させた)表示対象データだけを抜き出す (下書きは強制排除、下げる投稿は(設定に関係なく)表示対象)
	my @applydata = &extractApplyData();

	# 全データを走査して、指定No.の前後指定個数をリストアップして、あとは posts=123,124,125 のパラメータに置き換えてVIEWモードを実行すれば良い気がする。
	my @beforecenter = ();
	my @aftercenter = ();
	my $targetAppearFlag = 0;	# 中心番号に到達したフラグ

	foreach my $oneclip (@applydata) {
		# 投稿IDを得る
		...
		# 投稿番号をとりあえず格納する
		if( $targetAppearFlag == 0 ) {
			# 前半を収録している最中なら、前半として格納
			push( @beforecenter , $id );

			# 収録上限を超えたら最も古い要素(＝先頭要素)を削除
			if( ($#beforecenter + 1) > $around ) {
				# 超えていたら1件消す
				shift @beforecenter;
			}
		}
	...
	# 対象データがなければ
	if( $targetAppearFlag == 0 ) {
		&showadminpage('NO POST ID','',"<p>指定番号の投稿は存在しないか、または一覧表示の対象外か、または条件に合致していません。</p>",'B');
		exit;
	}

	# 配列をカンマで結合。中心番号もさらに結合。それを posts パラメータにする。
	$cp{'posts'} = join(',',@beforecenter) . ",$postid," . join(',',@aftercenter);
	&replaceparams();

	# 検索以外の指定条件パラメータをすべて(内部的には)空文字にして、VIEWモードを実行
	$cp{'hasgtag'} = '';
	$cp{'cat'} = '';
	$cp{'userid'} = '';
...
# ログイン者限定表示時のログイン確認 #
# ---------------------------------- #
sub loginCheckOnPrivateOperation
{
	my $permittedid = &fcts::checkpermission(1);	# 読み取り専用モードで認証を確認
	if( $permittedid eq '' ) {
		# ログインしていない場合は、ログイン要求メッセージだけを表示して終わる
		my $guidemsg = &fcts::trim($setdat{'privoploginmsg'});
		if( $guidemsg eq '' ) {
			# 独自メッセージがなければ、デフォルトのメッセージを表示
			$guidemsg = '<p>この「てがろぐ」は、<strong>ログイン者限定表示</strong>に設定されています。すべてのページは、<strong class="important">ログインしている状態でしか表示されません</strong>。</p><p>まずは、<a href="?mode=admin">ログインして下さい</a>。</p>';
		}
		else {
			# マークアップを加える
			$guidemsg = '<p>' . &fcts::forsafety( $guidemsg ) . '</p>';
		}
		&showadminpage('LOGIN REQUIRED','', $guidemsg . '<br>' ,'BL');
		exit;
	}
}
...
sub modeView
{
	...
	my $outputtype = shift @_ || 0;		# 引数2：出力タイプ (0=画面、1=ファイル)
	my $outputfile = shift @_ || '';	# 引数3：出力ファイルパス
	...
	if( $builtinskin eq '' ) {
		...
		if(( $setdat{'banskincomplement'} != 1 ) && ( $cvskin !~ m/^[-\[\]A-Z]*\[\[NO-COMPLEMENT\]\]/ )) {
			...
			if( $cvskin =~ m|<html|i ) {
				...
				elsif( $cvskin !~ m|<title>.*?\[\[SITUATION.*?</|i ) { $cvskin =~ s|<title>(.*?)</|<title>[[SITUATION:TITLE]] $1</|i; }
	...
	my %isExist = (
		...
		COMPLEXSEARCH	=> ( $cvskin =~ m/\[\[SEARCHBOX:COMPLEX/ ) || 0,	# 複合検索窓の掲載がある場合
		IMAGELIST		=> ( $cvskin =~ m/\[\[IMAGELIST/ ) || 0,		# 画像リストの掲載がある場合
	...
	# -----------------------------------
	# 表示限定メッセージ(SITUATION)を作る		※SITUATION:CLASSの値はIF文でも使うので、ここで生成しておく。
	...
	# ---------------------------
	# ▼スキン内にある IF文の処理		※IF文のカッコ内に含められるのは、英数字＋ハイフン・縦棒・空白のみ。
	# ---------------------------
	if( $cvskin =~ m/\[\[IF\([-\|%\w\s]+\):/ ) {
		# IF記法があれば、処理する関数へ渡す。(SITUATION:CLASSは既に生成済み)
		$cvskin =~ s/\[\[IF\(([-\|%\w\s]+)\):([\s\S]+?):IF\]\]/&processIFstatement( $1, $situationcls, $2 )/eg;		# 改行も含めて$2へ取り込む必要があるので . ではなく [\s\S] を使う
	}
	...
	# ----------------------------------------------------
	# 該当データを分解して(内側スキンから)表示用HTMLを作成
	my @cliphtmls = ();
	if( $#showdata >= 0 ) {
		# データが1件以上存在する場合のみ、内側スキンを解釈
		@cliphtmls = &expandDataBySkin( $needoneogp, $builtinskin, '', $situationcls, @showdata );
	}
	...
	if( $isExist{'NAVI'} ) {
		...
		if(( $cp{'postid'} > 0 ) && ( $setdat{'pagelinkuseindv'} == 1 )) {
			...
			# 表示対象限定クエリを作る
			my @limquery = &makeLimitQuery(2);

			# 隣接投稿の存在を探す
			my ($previdnum,$nextidnum) = &findNeighbours( $cp{'postid'} ,1 );	# 限定状況を考慮して隣を探す
			if( $previdnum > 0 ) {
				# 前の投稿があれば
				my $prevlink = &makeQueryString( @limquery, "postid=$previdnum" );
			...
			if( $nextidnum > 0 ) {
				# 次の投稿があれば
				my $nextlink = &makeQueryString( @limquery, "postid=$nextidnum" );
		...
		# 表示対象が限定されている際(または表示順の変更時)には「全体表示に戻る」リンクを作る
		if( $cp{'postid'} > 0 || $cp{'userid'} ne '' || $cp{'cat'} ne '' || $cp{'hasgtag'} ne '' || $cp{'search'} ne '' || $cp{'datelim'} ne '' || $cp{'order'} ne 'straight' ) {
		...
		$cvskin =~ s/\[\[NAVI:PREV\]\]/$prevpagelink/g;
		$cvskin =~ s/\[\[NAVI:NEXT\]\]/$nextpagelink/g;
		$cvskin =~ s/\[\[NAVI:TOPPAGE\]\]/$backtotoplink/g;
		$cvskin =~ s/\[\[NAVI:TOPPAGE:([A-Z]+)\]\]/&linkadjustbymode($1,$backtotoplink)/eg;
	...
	if( $isExist{'DATEBOX'} ) {
		...
		# 指定モード用リンクの出力
		$cvskin =~ s/\[\[DATEBOX:PULL:([A-Z]+)\]\]/&linkadjustbymode($1,$dateselectform)/eg;			# 日付検索窓
		$cvskin =~ s/\[\[DATEBOX:LIST:([A-Z]+)\]\]/&linkadjustbymode($1,$setdat{'datelisthtml'})/eg;	# 日付リンクリスト
	}
	...
	if( $isExist{'LATESTLIST'} ) {
		...
		$cvskin =~ s/\[\[LATESTLIST:([A-Z]+)\]\]/&linkadjustbymode($1,$setdat{'latestlisthtml'})/eg;
	}
	...
	if( $isExist{'HASHTAGLIST'} ) {
		...
		# 指定モード用リンクの出力
		$cvskin =~ s/\[\[HASHTAG:LIST:([A-Z]+)\]\]/&linkadjustbymode($1,$hashtagliststring)/eg;		# ハッシュタグ:リストを指定モード用リンクで挿入
		$cvskin =~ s/\[\[HASHTAG:PULL:([A-Z]+)\]\]/"$hashtagpullstring1$hashtagpullstring2a$hashtagpullstring3" . &linkadjustbymode($1,$hashtagpullstring4a)/eg;		# ハッシュタグ:プルダウンメニューを指定モード用リンクで挿入
		$cvskin =~ s/\[\[HASHTAG:PULL:JS:([A-Z]+)\]\]/"$hashtagpullstring1$hashtagpullstring2b$hashtagpullstring3" . &linkadjustbymode($1,$hashtagpullstring4b)/eg;	# ハッシュタグ:プルダウンメニューJS版を指定モード用リンクで挿入
	}

	# --------------------
	# カテゴリツリーの挿入
	if( $isExist{'CATEGORYTREE'} ) {
		if( $cvskin =~ m/\[\[CATEGORY:TREE:?(GALLERY|PICTS|SITEMAP|RSS|KEEPMODE)?\]\]/ ) {
			# デフォルト設定のツリーを出力するなら
			my $catTreeSource = &makeCategoryTree();			# 先にカテゴリツリーを生成しておく (同じツリーを何度も生成してしまわないように)
			$cvskin =~ s/\[\[CATEGORY:TREE\]\]/$catTreeSource/g;

			# 指定モード用リンクの出力
			$cvskin =~ s/\[\[CATEGORY:TREE:(GALLERY|PICTS|SITEMAP|RSS|KEEPMODE)\]\]/&linkadjustbymode($1,$catTreeSource)/eg;
		}
		# フラグ付きのカテゴリツリー:指定モード用リンクの出力(こっちを先に処理する必要がある)
		$cvskin =~ s/\[\[CATEGORY:TREE:(.+?):([A-Z]+)\]\]/&linkadjustbymode($2,&makeCategoryTree($1))/eg;
	...
	if( $isExist{'CATEGORYPULL'} ) {
		...
		# 指定モード用リンクの出力
		$cvskin =~ s/\[\[CATEGORY:PULL:([A-Z]+)\]\]/&linkadjustbymode($1,&makeCategoryPulldown(0))/eg;	# カテゴリ:プルダウンメニューを挿入
		$cvskin =~ s/\[\[CATEGORY:PULL:JS:([A-Z]+)\]\]/&linkadjustbymode($1,&makeCategoryPulldown(1))/eg;	# カテゴリ:プルダウンメニューJS版を挿入
	}
	...
	if( $isExist{'QUICKPOST'} ) {
		...
		elsif(( $setdat{'operationtype'} == 1 ) && ( $permittedid ne '' ) && ( &fcts::getUserDetail($permittedid,1) == 1 )) {
			# ログイン者限定表示動作の場合で、ログインしているが、Lv.1の場合
			$quickpost = q|<style>.Login-Required { display: none !important; } /* 権限Lv.1のIDでの新規投稿は制限されています。 */</style>|;
		}
		else {
	...
	# 画像一覧リンクの挿入 (一時適用スキンは無視する)
	my $ptm = '?mode=picts';
	$cvskin =~ s/\[\[PICTS:URL\]\]/$cginame$ptm/g;			# 相対パスだが(後処理で)一時適用スキンが追加されないようにCGI名を加えておく
	$cvskin =~ s/\[\[PICTS:URL:FULL\]\]/$cgifullurl$ptm/g;	# 絶対URL
	...
	if( $isExist{'CALENDAR'} ) {
		...
		# カレンダー:指定モード用リンクの出力
		$cvskin =~ s/\[\[CALENDAR:([A-Z]+)\]\]/&linkadjustbymode($1,&getCalendarBox($fyear,$fmonth,0))/eg;
		$cvskin =~ s/\[\[CALENDAR:(-?\d{1,4}):([A-Z]+)\]\]/&linkadjustbymode($2,&getCalendarBox($fyear,$fmonth,$1))/eg;
	...
	if( $cp{'cat'} ne '' ) {
		# カテゴリ限定表示時のみ実行
		$cvskin =~ s/\[\[INFO:NOWCAT:DESCRIPTION\]\]/&outputCategoryInfo( $cp{'cat'}, 'D' )/eg;
		$cvskin =~ s/\[\[INFO:NOWCAT:NAME\]\]/&outputCategoryInfo( $cp{'cat'}, 'T' )/eg;
		$cvskin =~ s/\[\[INFO:NOWCAT:ICON\]\]/&outputCategoryInfo( $cp{'cat'}, 'I' )/eg;
		$cvskin =~ s/\[\[INFO:NOWCAT:ID\]\]/&outputCategoryIDs( $cp{'cat'}, ' ' , '' )/eg;	# 空白区切り
	}
	else {
		# カテゴリ限定表示時以外なら空文字に置き換える
		$cvskin =~ s/\[\[INFO:NOWCAT:[A-Z]+\]\]//g;
	}
	...
	if( $setdat{'searchoption'} != 0 ) {
		...
		my $searchoptions = '';
		if( $cp{'userid'} ne '' ) {
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="| . &fcts::forsafety($cp{'userid'}) . qq|" name="userid" checked><span>表示中のユーザに限定して検索</span></label><br>|;
		}
		if( $cp{'hasgtag'} ne '' ) {
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="| . &fcts::forsafety($cp{'hasgtag'}) . qq|" name="tag" checked><span>表示中のハッシュタグに限定して検索</span></label><br>|;
		}
		if( $cp{'cat'} ne '' ) {
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="| . &fcts::forsafety($cp{'cat'}) . qq|" name="cat" checked><span>表示中のカテゴリに限定して検索</span></label><br>|;
		}
		if( $cp{'datelim'} ne '' ) {
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="| . &fcts::forsafety($cp{'datelim'}) . qq|" name="date" checked><span>表示中の日付範囲に限定して検索</span></label><br>|;
		}
		if( $cp{'mode'} eq 'gallery' ) {
			...
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="gallery" name="mode" checked><span>$galleryname内に限定して検索</span></label><br>|;
		}
		elsif( $cp{'mode'} eq 'picts' ) {
			my $pictsmodename = &fcts::forsafety($setdat{'pictsmodename'});
			if( $pictsmodename eq '' ) { $pictsmodename = '画像一覧'; }	# 名称が空文字だったら、とりあえず画像一覧と表記しておく。
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="picts" name="mode" checked><span>$pictsmodename内に限定して検索</span></label><br>|;
		}
		elsif( $cp{'mode'} eq 'sitemap' ) {
			my $sitemappageyname = &fcts::forsafety($setdat{'sitemappageyname'});
			if( $sitemappageyname eq '' ) { $sitemappageyname = 'サイトマップ'; }	# 名称が空文字だったら、とりあえずサイトマップと表記しておく。
			$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="sitemap" name="mode" checked><span>検索結果を$sitemappageynameとして表示</span></label><br>|;
		}
		# 検索オプションがあれば挿入する
		if( $searchoptions ne '' ) {
	...
	# 複合検索窓の挿入
	if( $cp{'mode'} eq 'picts' ) {
		# 画像一覧モードで表示された場合は、日付プルダウンメニューを臨時に再生成する
		&datadatecounter(1);
	}
	...
	if( $cvskin =~ /^([\W\w]*)\[\[TEGALOG:?\d*\]\]([\w\W]*)$/ ) {
		...
		# カバースキンを分割できなかった場合
		&errormsg('外側スキンにキーワード [[TEGALOG]] が含まれていないため、ページを生成できません。正しいスキンが指定されているかどうかを確認して下さい。<br>（もし、スキン内でIF文を使っている場合は、IF文の条件によって [[TEGALOG]] 自体の出力を抑制してしまっていないかも確認して下さい。）','<a href="?mode=admin">管理画面へ移動する</a>');
	}
	...
	if( $#cliphtmls < 0 ) {
		if( $cp{'postid'} > 0 ) {
			...
			elsif( $cp{'mode'} =~ m/^(gallery|picts)$/ ) {
				# ギャラリーモードか画像一覧モードなら
				push( @cliphtmls, '<div class="nodata nopost">指定された番号の投稿は存在しないか、または現在の表示モードでは表示対象外なために表示できません。</div>' );
			}
			else {
	...
	# ▼最終出力 (画面orファイル)
	# ---------------------------
	my $totaloutput = $coverfirst . join('',@cliphtmls) . $coversecond;
	...
	if( $outputtype == 1 ) {
		...
		# ▽ファイル出力の場合
		# --------------------
		if( &fcts::isvalidfilename( $outputfile ) ) {
			# 出力ファイル名がファイル名として成立するなら、そのファイル名で出力する
			&fcts::FILEout( $outputfile, $totaloutput );
		}
		else {
			# ファイル名がおかしい場合はエラー
			&errormsg('出力先ファイル名の指定' . &fcts::forsafety($outputfile) . 'が無効です。');
		}
	}
	else {
		# ----------------
		...
		# ------------------------------------
		# グランドホームなら index.html を出力
		if( $setdat{'outputindexhtml'} == 1 ) {
			# 出力する設定になっている場合だけ
			my $permittedid = &fcts::checkpermission(1);	# 読み取り専用モードで認証を確認
			if( $permittedid ne '' ) {
				# ログイン中の場合にだけ実行
				if(( &islimited() == 0 ) && ( $cp{'page'} == 1 ) && ( $cp{'mode'} eq 'view' ) && ( $cp{'skindir'} eq '' )) {
					# 表示条件が限定されていない状況で、Topページで、VIEWモードで、スキン指定がなければ、ここはグランドホーム
					# CGIへのリンク記述を修正
					$totaloutput =~ s|href="\?|href="$cginame\?|g;
					# ファイルに出力
					&fcts::FILEout( "index.html", $totaloutput );
				}
			}
		}
	}
...
# --------------------
# スキン内のIF文を処理		引数1：スキンに書かれたIFの条件文字列、引数2：SITUATION:CLASSの文字列、引数3：IF文によって出力するかどうかを決める文字列本体
# --------------------
sub processIFstatement
{
	...
	# 検索を実行
	if( &fcts::wordsearch( $situation , $condstr ) == 0 ) {
		# 非該当なら非出力コメントだけを返す
		return '<!-- IF文(' . &fcts::forsafety($condstr) . ')により非出力 -->';
	}
...
sub getpaths
{
	...
	elsif( $target eq 'SKINDIR:PICTS'	) { return &fcts::makelastcharslash( &fcts::relpathtoabspath( $basedir , &fcts::forsafety($setdat{'pictsmodeskindir'}) )); 		}	# ex: /tegalog/skin-picts/
...
sub makeimagelistbox
{
	...
	foreach my $oneopt ( @options ) {
		...
		elsif( $oneopt =~ m/(GALLERY|PICTS|SITEMAP)/ ) { $linkmode = $1; }	# mode
...
# 		my $user	= &fcts::forsafety( &fcts::getcontent($onerec,'user') );
		...
		if( $count < $showimgs ) {
			...
			# サムネイルの存在確認
			if( $setdat{'imageslistthumbfirst'} == 1 ) {
				# サムネイル画像があればサムネイルを表示する設定の場合だけ
				my $thumbpath = &fcts::makethumbnailpath( $fp, $subfile{'miniDIR'} );
				if( -f $thumbpath ) {
					# 存在すれば表示に使う (※この時点ではリンク等HTMLの出力は終わっているので $fp 変数の中身を直接書き換えても大丈夫なのでそうする)
					$fp = $thumbpath;
				}
			}
...
sub outputUserIcon
{
	...
	if( $imgsource eq '' ) {
		...
		# 指定画像のURLを調整
		if( $imgsource =~ m/^https?:\/\/.+/ ) {
			# もし指定画像が http:// か https:// で始まっているなら、ドメイン名を除いた絶対パス記法に変える
			if( &fcts::checkSameDomain( $cgifullurl, $imgsource ) ) {
				# CGI稼働URLと同じドメインなら
				$imgsource = &fcts::cutofftofullpath( $imgsource );		# 「/」で始まる絶対パスに変換
			}
		}
		# 指定の画像を表示
...
# --------------------------------		※製作メモ:この関数はほぼ「1回しか呼ばれない」ことを前提にしているので、2回以上呼ぶケースを増やす際には、何か特別な処理を足す必要がないか、よく考えること。
# 限定条件の該当投稿を抜き出す			引数1:postid指定を無視(1:する/0:しない)
# --------------------------------
sub extractApplyData
{
	my $ignorePostIdParam = shift @_ || 0;

	# PostIdの解釈を調整する
	my $cp_postid = $cp{'postid'};
	if( $ignorePostIdParam == 1 ) {
		# PostIdのパラメータを無視する場合
		$cp_postid = 0;
	}
	...
	if( $cp{'order'} eq 'reverse' ) {
		# 逆順なら
		if( $globalFlags{'alreadyReverseXml'} == 0 ) {
			# (まだひっくり返していないなら)データの配列を逆転
			@xmldata = reverse(@xmldata);
			$globalFlags{'alreadyReverseXml'} = 1;	# データをひっくり返したフラグを立てる
		}
	}
	...
	if( $nowlimited == 0 ) {
		...
		if(( $setdat{'fixedpostids'} ne '' ) && ( ($cp{'mode'} eq 'view') || ($cp{'mode'} eq 'sitemap') )) {
			&putpoststohead( $setdat{'fixedpostids'} );		# この関数で、グローバル変数 @xmldata の中身が書き換えられる点に注意。
		}
	}
	...
	foreach my $oneclip (@xmldata) {
		...
		if( $flags =~ m/rear/ ) {
			...
			elsif( $cp{'mode'} =~ /^around(\d*)$/ ) {
				# 隣接近傍一括表示モードの場合は、下げられている投稿も表示する
			}
			elsif(
...
# ----------------------------------------------
# 該当投稿に内側スキンを適用して表示用HTMLにする	引数1：OGP用の情報抽出が必要かどうか(1:必要/0:不要), 引数2：Built-inスキンを使う場合の名称, 引数3：動的スキンを使う場合はその中身、引数4：「SITUATION:CLASS」の値、引数5：表示する投稿データ
# ----------------------------------------------
sub expandDataBySkin
{
	...
	if( $inskin =~ m/\[\[IF\([-\|\w\s]+\):/ ) {
		# IF記法があれば、処理する関数へ渡す。(SITUATION:CLASSは既に生成済み)
		$inskin =~ s/\[\[IF\(([-\|\w\s]+)\):([\s\S]+?):IF\]\]/&processIFstatement( $1, $situationcls, $2 )/eg;		# 改行も含めて$2へ取り込む必要があるので . ではなく [\s\S] を使う
	}
	...
	foreach my $oneclip (@_) {
		...
		if( $setdat{'searchhighlight'} != 0 ) {
			...
			# ハイライトに使う要素を決める
			my $hiElmnt = 'strong';
			if( $setdat{'searchmarkup'} == 1 ) { $hiElmnt = 'span'; }
			elsif( $setdat{'searchmarkup'} == 2 ) { $hiElmnt = 'mark'; }
			elsif( $setdat{'searchmarkup'} == 3 ) { $hiElmnt = 'em'; }

			# ハイライト用のマークアップを用意する
			my $markup1 = '<' . $hiElmnt . ' class="searchword">';
			my $markup2 = '</' . $hiElmnt . '>';
		...
		if( $flagLock && ( $setdat{'secretkeystatus'} > 0 )) {
			...
			if( $showKeyForm ) {
				...
				if( $setdat{'allowonelineinsecret'} ) {
					...
					if( $comment =~ m|^(.*?)<br />| ) {
						# 本文内にbrタグがあるということは、1行を超える行数があるということなので、1行目だけを抜き出す。
						$line1 = $1 . '<br />';

						# さらに2行目も抜き出す設定かどうかを確認
						if( $setdat{'allow2ndlineinsecret'} ) {
							# 2行目も抜き出す設定なら、抜き出す
							if( $comment =~ m|^(.*?)<br />(.*?)<br />| ) {
								$line1 .= $2 .  '<br />';
							}
		...
		$mtemplate =~ s/\[\[P(E|A)RMAURL:KEEPCOND\]\]/						&makeQueryString( &makeLimitQuery(9) , "postid=$id")/eg;	# 表示限定クエリ付きのPERMAURL (維持条件はデフォルト)
		$mtemplate =~ s/\[\[P(E|A)RMAURL:KEEPCOND:FULL\]\]/$cgifullurl .	&makeQueryString( &makeLimitQuery(9) , "postid=$id")/eg;	# 同上
		$mtemplate =~ s/\[\[P(E|A)RMAURL:KEEPCOND\(([A-Z]+)\)\]\]/						&makeQueryString( &makeLimitQuery(9,$2) , "postid=$id")/eg;		# 表示限定クエリ付きのPERMAURL (維持条件はフラグ指定)
		$mtemplate =~ s/\[\[P(E|A)RMAURL:KEEPCOND\(([A-Z]+)\):FULL\]\]/$cgifullurl .	&makeQueryString( &makeLimitQuery(9,$2) , "postid=$id")/eg;		# 同上
		...
		$mtemplate =~ s/\[\[USERINTRO\]\]/&fcts::forsafety( &fcts::getUserDetail($user,3) )/eg;
...
sub cemoji
{
	...
	foreach my $ext (@exts) {
		...
		if( -f $tryfilepath ) {
			...
			my $clsatts = '';
			if( $cefname =~ m/[-._]/ ) {
				# ハイフン・ドット・アンダーバーが含まれていれば、分解してそれぞれをclass名にする
				$clsatts = ' ' . &makeCemojiPartsClass($cefname);
			}
			my $basecls = $cefname;
			$basecls =~ s/\.//g;		# ドットはclass名に含められないので消す
			$basecls = 'ce-' . $basecls;
			...
			# 生成したHTMLを返して終わる
			return qq|<span class="cemoji $basecls$clsatts"><img src="$usefilepath"$imgatts$limcss alt="$cefname"$cedcatt loading="lazy"></span>$cedcjs|;
		}
	}
...
# ハイフン・ドット・アンダーバーが含まれていれば、分解してそれぞれをカスタム絵文字用のclass名にする
sub makeCemojiPartsClass
{
	my $str = shift @_ || return '';

	# ハイフン・ドット・アンダーバーで分割して配列に格納
...
sub cutsuperfluouspartsforrssogp
{
	...
	# CodePen埋め込み部分の代替
	$comment =~ s|<span class="embeddedcodepen"><iframe.+?</iframe></span>|（CodePen埋め込み）|g;
	...
	foreach my $cid ( @cats ) {
		...
		foreach my $flag (split //, $flags) {
			...
			if(    $flag eq 'T' )	{ $catret .= qq|<span class="categoryname cat-$catid">$catname</span>|; }
				...
				# カテゴリ概要文の出力
				my $catdesc = &fcts::getCategoryDetail( $cid, 2 );	# 生のデータを取得
				if( $setdat{'allowhtmlondesc'} == 0 ) {
					# HTMLを許可しない設定なら安全化する
					$catdesc = &fcts::forsafety( $catdesc );
				}
				$catret .= qq|<span class="categorydescription cat-$catid">| . $catdesc . '</span>';
			}
			elsif( $flag eq 'I' )	{
				# カテゴリアイコンの出力
...
# -----------------------------------
# 文字列内に含まれるURLをリンク化する	※引数の文字列(＝本文全文)は既に安全化されていることが前提
# -----------------------------------
sub linkize
{
	my $ts = shift @_ || '';
	...
	if( $setdat{'urlexpandyoutube'} != 0 ) {
		...
		# Short動画・ライブ動画
		$ts =~ s{\[[Yy][Oo][Uu][Tt][Uu][Bb][Ee]\]https?:\/\/(?:www\.)?youtube\.com\/(?:shorts|live)/([-\w]+)[-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]*?\&?(t=\d+)?[-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]*}{&embedyoutube($1,$2)}eg;
	}
...
# 		$ts =~ s|\[[Ss][Pp][Oo][Tt][Ii][Ff][Yy]\]https?:\/\/open\.spotify\.com\/([A-Za-z]+?)/([-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]+)|&embedspotify($1,$2)|eg;
	...
	if( $setdat{'urlexpandtweet'} != 0 ) {
		...
		$ts =~ s{\[[Tt][Ww][Ee][Ee][Tt]\](https?:\/\/[\w\.]*(?:twitter|x)\.com\/[-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]*)}{&embedtweet($1)}eg;
	}
	...
	# ▼CodePenリンク処理
	if( $setdat{'urlexpandcodepen'} != 0 ) {
		$ts =~ s|\[[Cc][Oo][Dd][Ee][Pp][Ee][Nn]\]https?://[\w\.]*codepen\.io\/(\w+)/pen/(\w+)/?|<span class="embeddedcodepen"><iframe height="300" style="width: 100%;" scrolling="no" title="" src="https://codepen.io/$1/embed/$2?default-tab=html%2Cresult" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true"></iframe></span>|g;
	}

	# ▼ラベルがあればラベルでリンク化
	# 		※リンクラベルには半角角括弧 [ , ] と#以外なら何でも可とする。リンクラベルとして画像が指定されていれば、画像でリンクを作れる。
	# 		※カッコなしURLは相対パス無理。装飾記法の直後に「/」とか「..」とか出てくる場合に誤判定してしまうから。
	# 		※URLカッコ付きの場合、カッコの内側は http か / か ./ か ../ のどれかで始まる場合に限る。同階層の他ファイルを示したい場合は ./ で書き始める。
	if( $setdat{'urllabeledlink'} != 0 ) {
		# URLの直前に[ラベル]があればその文字列でリンクにする場合
		$ts =~ s|(?<!\\)\[([^\[\]]+?)\](https?:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]+)|&makelabeledlink($1,$2)|eg;		# 否定戻り読み (?<!\\) で、ラベル開始を示す [ が \[ になっていない場合だけを対象にする。
	}
	if( $setdat{'urlparenthesislink'} != 0 ) {
		# URLを丸括弧で囲む [ラベル](URL) 記法も使用可能にする場合
		$ts =~ s{(?<!\\)\[([^\[\]]+?)\]\(((https?://|\.{0,2}/|mailto:|tel:)[-_.!~*\'a-zA-Z0-9;\/?:@&=+\$,%#]+)\)}{&makelabeledlink($1,$2)}eg;		# URLの中に半角丸括弧は許容しない
	}
...
# ----------------------
# ラベル付きリンクを作る		引数1：リンクラベル（テキストとは限らず、画像表示HTMLソースの場合もある）、引数2：リンク先URL	※引数はどちらも安全化済み
# ----------------------
sub makelabeledlink
{
	my $linklabel = shift @_ || '';
	my $linkurl  = shift @_ || '';

	my $addatt = '';	# 追加属性群を入れる
	my $addclass = '';	# 追加class名を入れる

	# ラベル内にaタグが存在する場合はaタグを排除する (→画像をリンクラベルに含められる)
	$linklabel =~ s/<a .+?>(.+?)<\/a>/$1/g;

	# URLの中にさらに http が含まれる場合に、重複してリンク化してしまうのを防ぐために、http:// または https:// の記号を文字実体参照に変換しておく。
 	$linkurl =~ s|\:|&#58;|g;		# メモ：この文字実体参照は、sub comdecorateが実行されたときに元に戻される。(戻されなくても問題はないが。)
	...
	# ▽LBコマンド付きラベル
	if( $linklabel =~ m/:LB(:|$)/ ) {
		# コマンド部分は消す (※文字列の最後の方にある当該文字列を消すようにするため .+ を付けている)
		$linklabel =~ s/(.+):LB/$1/;
		# 独自のclass属性値を付加する設定なら加える
		if( $setdat{'urlimageaddclass'} == 1 ) {
			$addclass .= ' ' . &fcts::forsafety( $setdat{'urlimageclass'} );
		}
		# Lightbox用属性を加える
		$addatt .= " $setdat{'urlimagelightboxatt'}";
	}
...
# 	}
	...
	# nofollowの付加 (設定 or NFコマンド付きラベル)
	if(( $setdat{'urlnofollow'} == 1 ) || ( $linklabel =~ m/:NF(:|$)/ )) {
		$linklabel =~ s/(.+):NF/$1/;
		$addatt .= ' rel="nofollow"';
	}

	# target属性の付加
	if(( $setdat{'urllinktarget'} == 1 ) || ( $linklabel =~ m/:NEW(:|$)/ )) {
		# 新規ウインドウ
		$linklabel =~ s/(.+):NEW/$1/;
		$addatt .= ' rel="noopener noreferrer" target="_blank"';
	}
	elsif( $setdat{'urllinktarget'} == 2 ) { $addatt .= ' target="_top"'; 	}	# フレーム解除 (※ラベルに :NEW が付いている場合はそちらが優先される)

	# 属性の重複を解消
	$addatt =~ s/rel="nofollow" rel="noopener noreferrer"/rel="nofollow noopener noreferrer"/;

	return qq|<a class="url labeledlink$addclass" href="$linkurl"$addatt>$linklabel</a>|;
}
...
sub comdecorate
{
	...
	if( $setdat{'allowdecorate'} != 0 ) {
		...
		# 装飾用の要素名 (※通常は何らかの値が指定されているハズ。もし値が未設定な場合は、出力結果からその事実を判別しやすいように、デフォルト要素名を大文字で出力する)
		my $elmB = $setdat{'decoBtnElmntB'} || 'B';
		my $elmC = $setdat{'decoBtnElmntC'} || 'SPAN';
		my $elmD = $setdat{'decoBtnElmntD'} || 'DEL';
		my $elmE = $setdat{'decoBtnElmntE'} || 'EM';
		my $elmF = $setdat{'decoBtnElmntF'} || 'SPAN';
		my $elmH = $setdat{'decoBtnElmntH'} || 'SPAN';
		my $elmI = $setdat{'decoBtnElmntI'} || 'I';
		my $elmM = $setdat{'decoBtnElmntM'} || 'SPAN';
		my $elmQ = $setdat{'decoBtnElmntQ'} || 'Q';
		my $elmS = $setdat{'decoBtnElmntS'} || 'SMALL';
		my $elmT = $setdat{'decoBtnElmntT'} || 'SMALL';
		my $elmU = $setdat{'decoBtnElmntU'} || 'U';

		# 装飾ループ1
		for( my $i = 0; $i < $count ; $i++ ) {
			# 太字記述があれば(Bold)
			$ts =~ s|\[B:([^\[\]]+)\]|<$elmB class="decorationB">$1</$elmB>|g;
			# 色指定記述があれば(Color)
			$ts =~ s|\[C:([a-fA-F0-9]{6}):([^\[\]]+)\]|<$elmC class="decorationC" style="color:#$1;">$2</$elmC>|g;
			$ts =~ s|\[C:([a-fA-F0-9]{3}):([^\[\]]+)\]|<$elmC class="decorationC" style="color:#$1;">$2</$elmC>|g;
			$ts =~ s|\[C:([a-z]+):([^\[\]]+)\]|<$elmC class="decorationC" style="color:$1;">$2</$elmC>|g;
			$ts =~ s|\[C:(rgba?\([0-9\.,]+\)):([^\[\]]+)\]|<$elmC class="decorationC" style="color:$1;">$2</$elmC>|g;
			# 削除記述があれば(Delete)
			$ts =~ s|\[D:([^\[\]]+)\]|<$elmD class="decorationD">$1</$elmD>|g;
			# 強調記述があれば(Emphasis)
			$ts =~ s|\[E:([^\[\]]+)\]|<$elmE class="decorationE">$1</$elmE>|g;
			# 自由装飾指定記述があれば(Free)
			$ts =~ s{\[F:([-\w]+)(&#58;|:)([^\[\]]+)\]}{<$elmF class="decorationF deco-$1">$3</$elmF>}g;		# 正規表現の区切り記号として s||| の代わりに s{}{} を使用。＋『 [英:英数: 』に該当しない場合は2個目のコロンはエスケープされているので。
			# 斜体記述があれば(Italic)
			$ts =~ s|\[I:([^\[\]]+)\]|<$elmI class="decorationI">$1</$elmI>|g;
			# リスト記述があれば(List)
			$ts =~ s|\[L:([^\[\]]+)\]|&decoratelist($1)|eg;
			# マーカー指定記述があれば(Marker)
			$ts =~ s|\[M:([a-fA-F0-9]{6}):([^\[\]]+)\]|<$elmM class="decorationM" style="background-color:#$1;">$2</$elmM>|g;
			$ts =~ s|\[M:([a-fA-F0-9]{3}):([^\[\]]+)\]|<$elmM class="decorationM" style="background-color:#$1;">$2</$elmM>|g;
			$ts =~ s|\[M:([a-z]+):([^\[\]]+)\]|<$elmM class="decorationM" style="background-color:$1;">$2</$elmM>|g;
			$ts =~ s|\[M:(rgba?\([0-9\.,]+\)):([^\[\]]+)\]|<$elmM class="decorationM" style="background-color:$1;">$2</$elmM>|g;
			# 引用記述があれば(Quote)
			$ts =~ s|\[Q:([^\[\]]+)\]|<$elmQ class="decorationQ" style="display:block;">$1</$elmQ>|g;
			# ルビ記述があれば(Ruby)
			...
			# 小記述があれば(Small)
			$ts =~ s|\[S:([^\[\]]+)\]|<$elmS class="decorationS">$1</$elmS>|g;
			# 極小記述があれば(Tiny)
			$ts =~ s|\[T:([^\[\]]+)\]|<$elmT class="decorationT">$1</$elmT>|g;
			# 下線記述があれば(Underline)
			$ts =~ s|\[U:([^\[\]]+)\]|<$elmU class="decorationU">$1</$elmU>|g;

			# 隠す記述があれば(Hide)
			if( &canUseReadmoreFunc('H') == 1 ) {
				# 続きを読む機能を使う設定＆状況でのみ実行
				my $hideOpen = qq|<$elmH class="decorationH">|;
				my $hideClose = qq|</$elmH>|;

				# ▽[H:ボタンラベル(改行空白以外):隠す本文]記法
				if( $setdat{'readmoredynalabel'} == 1 ) {
					# 記法が使用可能な設定なら(※ラベルがある場合もない場合もこの正規表現1つで判定して置換)
					$ts =~ s{\[H:(([^\r\n\[\]<>\t\&]{1,100}?)(&#58;|:))?([^\[\]]+)\]}{$hideOpen . &outputHiddenArea('範囲を隠す' ,'' ,'' ,'' ,$elmH ,$setdat{'readmorestyle'} ,($2 || $setdat{'readmorebtnlabel'}) ,$setdat{'readmorecloselabel'} ,$setdat{'readmorecloseuse'} ,$4) . $hideClose}eg;	# ラベル文字列の安全化は送り先関数の方でしている # 正規表現の区切り記号として s||| の代わりに s{}{} を使用。
				}
				else {
	 				# ▽[H:隠す本文]記法だけを適用する場合
	 				$ts =~ s|\[H:([^\[\]]+)\]|$hideOpen . &outputHiddenArea('範囲を隠す' ,'' ,'' ,'' ,$elmH ,$setdat{'readmorestyle'} ,$setdat{'readmorebtnlabel'} ,$setdat{'readmorecloselabel'} ,$setdat{'readmorecloseuse'} ,$1) . $hideClose|eg;
				}
			}
			else {
				# 使えない状況なら全文を表示
...
sub decoratelist
{
	...
	# 第2オプション(＝任意class名のみ)があれば処理
	if( $body =~ m/^([0-9a-zA-Z]+)(:|&#58;)(.+)/ ) {
		# コロンより前が英数字だけならclass名だと解釈する
		$attclass .= ' listdeco-' . $1;
		# 残りが本当の本文部分
		$body = $3;
	}
...
sub utilitylinksforonepost
{
	...
	# 隣接一括表示リンク
	# ------------------
	if( $setdat{'utilityneighbor'} == 1 ) {
		# 何件表示するかを確認
		my $neinum = $setdat{'utilityneinum'};
		if( $neinum !~ /^[1-9]$/ ) {
			# 指定値が1～9の1桁ではない場合は強制的に修正
			$neinum = 3;
		}
		my $elink = &makeQueryString("mode=around$neinum","postid=$id");
		$ret .= qq|<li class="uneighbor"><a href="$elink">この投稿に隣接する前後$neinum件ずつをまとめて見る</a></li>|;
	}
...
sub urlshorter
{
	...
	# nofollowの付加
	if( $setdat{'urlnofollow'} == 1 ) { $addatt .= ' rel="nofollow"'; }

	# target属性の付加
	if( $setdat{'urllinktarget'} == 1 ) { $addatt .= ' rel="noopener noreferrer" target="_blank"'; }	# 新規ウインドウ
	if( $setdat{'urllinktarget'} == 2 ) { $addatt .= ' target="_top"'; 	}								# フレーム解除

	# 属性の重複を解消
	$addatt =~ s/rel="nofollow" rel="noopener noreferrer"/rel="nofollow noopener noreferrer"/;

	return qq|<a class="url" href="$url"$addatt title="$url">$sho</a>|;
}
...
sub embedtweet
{
	...
	# デモモード下の制限
	if( $flagDemo{'AddRelNofollow'} == 1 ) {
		if( $ytid !~ m|/nishishi/status/\d+| ) {
			# 作者ツイート以外だったら拒否
			return qq|<a href="$ytid">Twitterで見る</a> <i style="| . &styleforembeddederror() . q|">デモモードで動作しています。ここでは、作者であるnishishiアカウント以外のツイートの埋め込みはできません。テキストリンクとして掲載は可能ですが、ツイート主が拡散を希望していない可能性がある点にもご配慮下さい。</i>|;
		}
	}
	...
	$ytid =~ s|x\.com|twitter.com|;

	# mobile等のサブドメインを削除する
	$ytid =~ s|^(https?://)\w+\.(twitter\.com)|$1$2|;

	# フェイルセーフ用リンクへtarget属性を付加
	my $addtarget = '';
	if( $setdat{'urllinktarget'} == 1 ) {
		# 新規ウインドウ
		$addtarget .= ' target="_blank"';
	}
	elsif( $setdat{'urllinktarget'} == 2 ) {
		# フレーム解除
		$addtarget .= ' target="_top"';
	}

	# ツイート埋め込み用のHTMLにして返す (Do not trackオプションは標準で含めておく)
	return qq|<blockquote class="twitter-tweet" data-dnt="true"$twtheme>（ツイート埋め込み処理中...）<a href="$ytid"$addtarget>Twitterで見る</a></blockquote>$retjs|;
}
...
sub getDateSeparator
{
	...
	# 日付以外の限定条件もあればクエリに加える (※dateやorderの調整が必要なので makeLimitQuery は使えない。)
	...
	# GALLERY、PICTS、SITEMAPモードの場合は、読むリンクにはモード指定文字列も加える
	if( $cp{'mode'} =~ m/^(gallery|picts|sitemap)$/ ) {
		$urlforread .= '&amp;mode=' . $cp{'mode'};
	}
...
sub makeLimitMsg
{
	...
	if( $cp{'mode'} eq 'gallery' ) {
		...
		# 画像一覧モードの場合
		$specialmode = &fcts::forsafety( $setdat{'pictsmodename'} ) . ' ';	# 画像一覧ページ名称
		if( $setdat{'pictsmodesituation'} == 1 ) {
			$limitmsg = ' ';	# 空白で区切る
		}
	}
	elsif( $cp{'mode'} eq 'sitemap' ) {
...
# ------------------------
# 表示対象限定クエリを作る	引数1:除外する限定項目(0=何も除外しない／1=「mode」を除外する／2=「postid」を除外する／9=指定オプションに従う)、引数2:維持する項目フラグ群
# ------------------------
sub makeLimitQuery
{
	my $exclude = shift @_ || 0;
	my $keepflags = shift @_ || 'MUCHSD';	# M:モード、U:ユーザ限定、C:カテゴリ限定、H:ハッシュタグ限定、S:検索語、D:日付指定、*O:表示順序	(*=デフォルトでは含まない)

	# -----------------------------
	# 採用フラグ群の準備 (usec = USE Cond)
	my %usec = (
		M => 1,
		P => 1,
		U => 1,
	...
	if( $exclude == 1 ) {
		...
		$usec{'D'} = 0;
		$usec{'O'} = 0;
		foreach my $flag (split //, $keepflags) {
			# フラグがあるだけループ (※変数にフラグ以外の文字列が含まれているときに余計な処理をしてしまわないよう、あえて1文字ずつ一致を確認する。)
			if( $flag eq 'M' )	{ $usec{'M'} = 1; }
			if( $flag eq 'P' )	{ $usec{'P'} = 1; }
			if( $flag eq 'U' )	{ $usec{'U'} = 1; }
			if( $flag eq 'C' )	{ $usec{'C'} = 1; }
			if( $flag eq 'H' )	{ $usec{'H'} = 1; }
			if( $flag eq 'S' )	{ $usec{'S'} = 1; }
			if( $flag eq 'D' )	{ $usec{'D'} = 1; }
			if( $flag eq 'O' )	{ $usec{'O'} = 1; }
		}
	}
	...
	if( $usec{'M'} ) {
		if( $cp{'mode'} ne 'view' ) {
			# VIEWモード以外なら、そのモードを維持するパラメータを加える
			my $nowmode = &fcts::forsafety( $cp{'mode'} );
			push(@lq, "mode=$nowmode");
		}
	}

	# 投稿限定（※IDはユニークな番号なので、正しいデータで動作している限り、このコードが実行されることはない。＝同じIDの投稿が複数は存在しない。）
	if(( $usec{'P'} ) && ( $cp{'postid'} > 0 )) {
		push(@lq, ("postid=" . &fcts::forsafety($cp{'postid'}) ));
	}

	# ユーザ名限定
	if(( $usec{'U'} ) && ( $cp{'userid'} ne '' )) {
	...
	# 日付指定限定
	if(( $usec{'D'} ) && ( $cp{'datelim'} ne '' )) {
		push(@lq, ("date=" . &fcts::forsafety($cp{'datelim'}) ));
	}

	# ページ移動リンクに恒常系設定を付加
	if(( $usec{'O'} ) && ( $cp{'order'} eq 'reverse' )) {
		# 逆順の場合：
...
sub modeEdit
{
	...
	# -------------------------------------
	# スキンDIRに edit.htm があれば読み込む
	# -------------------------------------
	my $edithtm = '';
	if( $setdat{'loadedithtm'} == 1 ) {
		# 読み込む設定の場合だけ
		my $ehfilepath = &fcts::joinwithslash( $status{'skindir'} , $subfile{'editHTM'} );		# ファイルパスを作る(現在スキンのDIR / ファイル名)
		if( -f $ehfilepath ) {
			# edit.htmがあれば読み込む
			$edithtm = &fcts::FILEin( $ehfilepath, 1 );
		}
		else {
			# ない場合
			$edithtm = "<!-- $subfile{'editHTM'}を埋め込む設定ですが、 $ehfilepath が見つかりませんでした。 -->";
		}
	}
	...
	my $msg;
	if( $permitforedit > 0 ) {
		$msg = &makepostform( $id, $user, $date, $comment, $cats, $flags, '' ) . $editperson;		# 投稿フォームHTMLの作成関数を呼んでいるのはここ
	...
	my $cssform = q|
		...
		.postform p { margin:0; }
		textarea.tegalogpost { border:2px green solid; border-radius:0.67em; background-color:white; padding:0.5em; font-size:1rem; box-sizing:border-box; width:100%; height:8.6em; overflow-wrap:break-word; overflow:auto; margin-bottom:0.25em; form-sizing: content; }
...
sub modeWrite
{
	...
	# ゲストIDの場合で、ゲストIDが読み専に設定されている場合は拒否する
	if(( $plv == 1 ) && ( $setdat{'guestidtype'} == 1 )) {
		&showadminpage('READ ONLY','','<p>現在の設定では、<strong class="important">ゲストIDでの投稿はできません</strong>。</p><p>※投稿したい場合は、Lv.3以上の権限を持つIDでログインし直して下さい。</p>','BO');
		exit;
	}
	...
	if( $existnewdata == 1 || defined($ids[0]) ) {
		# ▼年月再集計
		&datadatecounter();
		push(@accompanies,'年月別該当個数を再集計しました。');
		# ▼新着リストの再生成
		&updatelatestlist( &makelatestlist( $setdat{'latestlistup'} , $setdat{'latestlistparts'} , $setdat{'latesttitlecut'} ));
		push(@accompanies,'新着リストを再生成しました。');
		# ▼カテゴリ再集計 (カテゴリが選択されていた場合 または 投稿削除の場合)
		if( $#newcats >= 0 || defined($ids[0]) ) {
			&categorycounter();
			push(@accompanies,'カテゴリ選択数を再集計しました。');
		}
		...
		if( $newcomment =~ m/#/ || defined($ids[0]) ) {
			...
			push(@accompanies,'ハッシュタグの該当個数を再集計しました。');
		}
		# ▼先頭固定状況をいじった場合
		if( $resfixed ne '' ) {
			push(@accompanies, $resfixed );
		}
	}
	...
	if( $setdat{'outputsitemapxml'} == 1 ) {
		# 出力する場合
		&outputXmlSitemap( 1, $subfile{'sitemapXml'} );
		push(@accompanies,"SITEMAP XMLを $subfile{'sitemapXml'} に静的出力しました。");
	}
	...
	# 結果を表示
	&showadminpage('EDITING COMPLETED','',$msg,'CNA',$css);
}
...
sub isfixed
{
	...
	@nowfixes = &fcts::alltrim(@nowfixes);	# すべてをtrimする
	...
	$msg .= q|
		<div class="basicinfo">
			<fieldset id="basicinfo">
				...
				<p class="fieldsubset">《現在の連携コード》 | . &fcts::forsafety($meta{'COOPCODE'}) . q| <a href="?mode=admin&amp;work=setting&amp;page=4#sysAdvanced" class="btnlink">設定/変更</a></p>
			</fieldset>
		</div>
		<p class="noticebox">
...
# ADMIN：読み専ID用初期画面
# -------------------------
sub menuForReadOnlyId
{
	my $username = shift @_ || '';	# アクセスユーザ名
	my $timeoutlim = shift @_ || '';	# セッション期限

	my $backlabel = &fcts::forsafety( $setdat{'conpaneretlinklabel'} );	# てがろぐHOMEへ戻るラベル
	my $homeItem	= &genSysMenuItem($backlabel,	'Back to Web',		'HOMEページへ戻ります。',	&makeQueryString(''),	1, '');
	my $logoutItem	= &genSysMenuItem('ログアウト',	'Logout',	'ログアウトします。',		&makeQueryString('mode=admin','work=logout'),	1, '');

	my $msg = '<p>' . $username . 'さん、ログインに成功しました。あなたのIDでは、読むことだけができます。（投稿や編集はできません。）</p><ul class="systemmenu"><li>' . $homeItem . '</li> <li>' . $logoutItem . '</li></ul><p class="noticebox">※ログアウトせずにブラウザを終了した場合、' . $timeoutlim . '後まではログイン状態が維持されます。<br>※投稿するには、Lv.3以上の権限のあるIDでログインする必要があります。または、ゲストIDでも投稿可能にするには、管理者がシステム設定を変更する必要があります。</p>';

	&showadminpage('LOGIN SUCCEEDED','',$msg,'CO');
	exit;
}
...
sub adminMenu
{
	...
	my $timeoutlim = '0秒';
	if( $keepsession == 1 ) {
		$timeoutlim = &fcts::sectotimestring( $sessiontimeout );
	...
	# ゲストIDの場合で、ゲストIDが読み専に設定されている場合は専用の画面だけを見せる
	if(( $permitlevel == 1 ) && ( $setdat{'guestidtype'} == 1 )) {
		&menuForReadOnlyId($username,$timeoutlim);
		exit;
	}
	...
	my $envVerInfo = '<p class="envVer">Perl ' . &fcts::PerlVersion() . ' <small>(CGI.pm v' . $cgi_version . ')</small></p>';
	...
	# .htaccessファイルがない場合の注意喚起
	if(! -f '.htaccess' ) {
		# .htaccessファイルがない場合
		$msg .= qq|<p class="alertnotice">※CGI本体と同じディレクトリに <a href="$aif{'puburl'}setup/#htaccessfile">.htaccess ファイル</a> が見つかりません。動作に必須ではありませんが、できるだけ設置することをお勧め致します。特に、予約投稿、下書き機能、鍵付き機能などの、<a href="$aif{'puburl'}setup/#htaccessfile-required">隠す系統の機能を使う場合には必要</a>です。</p>|;
	}
	...
	my $css = '<style>
		...
		.envVer { text-align:center; color:#aaa; font-size:0.75em; margin:0.75em 0; }
		.alertnotice { border:1px solid crimson; background-color:#fee; font-size:0.8em; padding:0.67em; border-radius:0.4em; }
		@media all and (min-width: 600px) {
			/* 管理画面 */
...
sub vui
{
	...
	if( $oplink ) {
		# リンク先を用意する
		my $linkto = "$aif{'puburl'}setup/#howtovup";
		# バージョンアップツールの設置有無を確認
		if( -f $subfile{'verupTool'} ) {
			# あればリンク先をそれに変更
			$linkto = $subfile{'verupTool'} . '?from=tegalog';
		}
		# 出力
		$ret = qq|<a href="$linkto" class="nowversion">| . $ret . '</a>';
	}
...
sub adminCategories {
	...
	# カテゴリページ上部
	my $msg = "<p>カテゴリの一覧を確認したり、新規に追加したり、編集したりします。カテゴリを新設する場合は「新規にカテゴリを作成」ボタンを押して下さい。編集する場合は、カテゴリIDをクリックして下さい。削除する場合は、下部の削除フォームを使って下さい。（※各投稿の表示に使われるカテゴリの区切り文字等の表示に関しては、[<a href=\"?mode=admin&amp;work=setting\">設定</a>]→[ページの表示]→[<a href=\"?mode=admin&amp;work=setting#pCategory\">カテゴリの表示</a>]から設定できます。）</p>";

	# 画面レイアウト
	$msg .= qq|\n<div class="categoryCtrlPanel">\n|;
	...
	if( $dellist ) {
		...
		if( $setdat{'allowhtmlondesc'} == 1 ) {
			# 概要文中のHTMLが許可されている場合
			$addmsg = '<br>※概要文中でHTMLの使用を許可する設定になっていますが、この画面上ではHTMLは解釈されず、そのままソースが表示されます。' . $addmsg;
		}
	...
	my $css = q|<style>
		...
		.categoryTableBox h2 { margin-top: 0; line-height:1; }
		table.catlist { line-height:1; }
		table.catlist th { white-space: nowrap; font-size:0.85em; padding:0.5em 0.2em; font-weight:normal; }
		table.catlist td { padding: 0.5em 0.4em; }
		.categoryTreeBox {
			margin: 0;
			padding: 0 1em;
			min-width: 240px;
		...
		@media (max-width: 1079px) {
			...
			table.catlist th { white-space: normal; }
		}
...
sub makeCategoryTree
{
	...
	# 概要文にHTMLを許可するかどうか
	my $allowhtmlopt = 0;
	if( $setdat{'allowhtmlondesc'} == 1 ) { $allowhtmlopt = 1; }

	# カテゴリツリーを得る
	my $ctree .= &fcts::getCategoryTree('',$opt ,$allowhtmlopt);
...
sub adminCatEditor
{
	...
	# 概要文中のHTMLが許可されている場合の追加表示
	my $allowhtmlsign = '';
	if( $setdat{'allowhtmlondesc'} == 1 ) {
		$allowhtmlsign = '※HTML使用可';
	}
		...
		<form action="$cgipath" method="post"><input type="hidden" name="mode" value="admin"><input type="hidden" value="trychangecat" name="work"><input type="hidden" name="idwork" value="$work">
			<fieldset>
				...
				<ul class="inputs">
					...
					<li><label><span class="itemhead">概要文：    </span> <input type="text" value="$cdscr" name="trydesc"></label> <span class="notice">※いつでも変更できます。(省略可) $allowhtmlsign</span></li>
...
sub adminImageMeta
{
	...
	# パラメータのファイル名を得る
	my $paramfilename = $cgi->param('img') || '';
	if( $paramfilename eq '' ) {
		# パラメータがなければエラー
	...
	# 指定画像のデータをインデックスから得る
	my ($res,$imeta_ref) = &getOneMetaFromImageIndex( $paramfilename );
	if( $res == 0 ) {
		# 対象画像がない場合
	...
	# ファイルパスを作る
	my $filepath	= "$imagefolder/$paramfilename";						# メイン画像
	my $thumbpath	= "$imagefolder/$subfile{'miniDIR'}/$paramfilename";	# サムネイル

	# 画像サイズを得る
	my ($iwidth,$iheight) = &fcts::getImageWidthHeight($filepath);
	...
	my $thumbbox = '';
	if( -f $thumbpath ) {
		# サムネイル用画像ファイルが存在するなら、ファイルパスとリンクの表示
		my $tfp = &fcts::forsafety($thumbpath);	# サムネイルパスを表示用に安全化
		$thumbbox = &fcts::tooneline( qq|
			<tr><th>サムネイル：</th><td class="thumbbox">
				<span id="tmbinfo">
					<a href="$tfp" data-lightbox="tegalogimage" data-title="$tfp">$tfp</a>
				</span>
			</td></tr>
		|);
	}

	# 表示用に安全化(ファイル名・ファイルパスを)
	my $fn = &fcts::forsafety($paramfilename);
	my $fp = &fcts::forsafety($filepath);
	...
	my $cgipath = &getCgiPath();
	$msg .= qq|
		<p>画像ファイル [ <a href="$fp" class="imagetextlink">$fn</a> ] の情報を編集します。</p>
		<div class="imagecover">
			...
			<div class="metacol">
				<form action="$cgipath" method="post">
					...
					<p class="btnbox">
						<input type="hidden" value="$fn" name="img">
	...
	my $css = q|<style>
		...
		.tmbctrlbtn { font-size:0.7em; vertical-align:bottom; line-height:1; padding:0 0.5em; }
		.tmbimgdelguide { font-size:0.67em; color:gray; }
...
sub adminManage
{
	...
	my $resetFiltering = '';
	if( $searchInput ne '' ) {
		# 絞り込み中なら、その旨の表示に書き換えて、リセット用リンクを出力する
		$searchTitle = q|<span class="filtered">絞り込み中</span>|;
		$resetFiltering = q|<a href="#" class="filterReset" onclick="var filteringBox = document.getElementById('filteringBox'); filteringBox.value = ''; filteringBox.form.submit(); return false;">絞込解除</a>|;
	}
	...
	my $restable = qq|
		...
		<div class="headForm">
			...
			<form action="$cgipath" method="get">
				▼$searchTitle：<input type="search" name="q" value="$searchInput" placeholder="任意の検索語・カテゴリID・投稿日付等" class="filteringBox" id="filteringBox">
				<input type="submit" value="検索"> $resetFiltering
				<input type="hidden" value="admin" name="mode">
	...
	foreach my $oneclip (@xmldata) {
		...
		if(( $plv >= 7 ) || ( $user eq $permittedid )) {
			...
			if( $showflag ) {
				...
				# 編集画面のURLを作る
				my $editurl = &makeQueryString('mode=edit',"postid=$id");
				# 追加
				my $addhtml = &fcts::tooneline(
					qq|
						<tr$lineclass>
							<td class="mtid"><label class="choiceid"><input type="checkbox" name="postid" value="$id">$id</label></td>
							<td class="mttitle"><a href="$postlink" class="mttlink">$comstr</a></td>
							<td class="mttime">$date</td>
							<td class="mtcat">$flaglinks$cats</td>
							<td class="mtuser"><a href="$userlink">$user</a></td>
							<td><a href="$editurl" class="smallbutton">Edit.$id</a></td>
						</tr>
					|
				) . "\n";
				push( @posttable, $addhtml );
				$filteredposts++;
			}
			$permittedposts++;
		}
...
sub deletedecos
{
	...
	$str =~ s{(?<!\\)\[([^\[\]]+?)\]\(((https?://|\.{0,2}/|mailto:|tel:)[-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]+)\)}{(リンク:$1)}g;		# カッコ付きURLリンク
	$str =~ s|\[(.+?)]https?://[\w!\?/\+\-_~=;\.,\*&@#\$%\(\)'\[\]]+|(リンク:$1)|ig;	# リンク記法を置き換え (あらゆるリンク記法より後に実行する必要がある)

	# 各種埋め込みの場合は、サービス名だけを出すよう修正
	$str =~ s|\(リンク:Tweet\)|(ツイート)|ig;
	$str =~ s{\(リンク:(YouTube|Instagram|Insta|CodePen|AppleMusic|Spotify)\)}{($1)}ig;
...
sub adminSetting
{
	...
	my @hashtagspace = ('','','');			$hashtagspace[$setdat{'hashtagspace'}] = 'selected';
	my @readmorestyle = ('','','');			$readmorestyle[$setdat{'readmorestyle'}] = 'selected';
	my @searchmarkup = ('','','','');		$searchmarkup[$setdat{'searchmarkup'}] = 'selected';
	...
	# 文字装飾記法用の出力要素選択用プルダウンメニューを(設定状況を反映させつつ)作る
	my %decoBtnElmnt = ();
	my @decoBtns = ('B','C','D','E','F','H','I','M','Q','S','T','U');
	my @inlines = &fcts::getStandaloneElementList('inline');
	my @blocks = &fcts::getStandaloneElementList('block');
	foreach my $oneBtn ( @decoBtns ) {
		# 設定項目名(連想配列名)
		my $keyname = 'decoBtnElmnt' . $oneBtn;
		# select開始タグを作る
		my $selectopen = '<select name="' . $keyname . '" class="elementSelect">';
		# optionタグを作る
		my $optiontags = '<option value="">標準に戻す</option>';
		my $selected = '';
		foreach my $oneElmnt ( @inlines ) {
			# 設定値と一致する項目を選択済みにする
			if( $setdat{$keyname} eq $oneElmnt ) { $selected = ' selected'; }
			else { $selected = ''; }
			# 選択肢を作る
			$optiontags .= '<option value="' . $oneElmnt . '"' . $selected . '>' . $oneElmnt . '</option>';
		}
		foreach my $oneElmnt ( @blocks ) {
			# 設定値と一致する項目を選択済みにする
			if( $setdat{$keyname} eq $oneElmnt ) { $selected = ' selected'; }
			else { $selected = ''; }
			# 選択肢を作る
			$optiontags .= '<option value="' . $oneElmnt . '"' . $selected . ' class="noticeElm">' . $oneElmnt . ' (注)</option>';
		}
		# 各文字装飾記法用の出力要素選択用プルダウンメニューにする
		$decoBtnElmnt{$oneBtn} = $selectopen . $optiontags . '</select>';
	}
	...
	my @pictsmodenolisted = ('','');		$pictsmodenolisted[$setdat{'pictsmodenolisted'}] = 'checked';
	...
	my @operationtype = ('','');			$operationtype[$setdat{'operationtype'}] = 'checked';
	...
	$setdat{'utilityneighbor'}		= &getattributeforcheckbox($setdat{'utilityneighbor'});
	...
	$setdat{'pictsmodeoutput'}		= &getattributeforcheckbox($setdat{'pictsmodeoutput'});
	$setdat{'pictsmodedatebar'}		= &getattributeforcheckbox($setdat{'pictsmodedatebar'});
	$setdat{'pictsmodesituation'}	= &getattributeforcheckbox($setdat{'pictsmodesituation'});
	...
	$setdat{'urllabeledlink'}		= &getattributeforcheckbox($setdat{'urllabeledlink'});
	$setdat{'urlparenthesislink'}	= &getattributeforcheckbox($setdat{'urlparenthesislink'});
	...
	$setdat{'urlexpandcodepen'}		= &getattributeforcheckbox($setdat{'urlexpandcodepen'});
	...
	$setdat{'allow2ndlineinsecret'}	= &getattributeforcheckbox($setdat{'allow2ndlineinsecret'});
	...
	$setdat{'outputsitemapxml'}		= &getattributeforcheckbox($setdat{'outputsitemapxml'});
	$setdat{'outputindexhtml'}		= &getattributeforcheckbox($setdat{'outputindexhtml'});
	$setdat{'loadeditcssjs'}		= &getattributeforcheckbox($setdat{'loadeditcssjs'});
	$setdat{'loadedithtm'}			= &getattributeforcheckbox($setdat{'loadedithtm'});
	$setdat{'allowhtmlondesc'}		= &getattributeforcheckbox($setdat{'allowhtmlondesc'});
	...
	$setdat{'guestidtype'}			= &getattributeforcheckbox($setdat{'guestidtype'});
	...
	if( $setdat{'operationtype'} eq '' ) { $setdat{'operationtype'} = '0'; }
			...
			<ul class="list">
				...
				<li><input type="checkbox" name="onepostpageutilitybox" id="onepostpageutilitybox" value="1" $setdat{'onepostpageutilitybox'}><label for="onepostpageutilitybox">本文の下に、ユーティリティリンク枠を表示する</label>
					<ul class="list">
						...
						<li><label><input type="checkbox" name="utilityneighbor" value="1" $setdat{'utilityneighbor'}>隣接する前後各<input type="number" value="$setdat{'utilityneinum'}" name="utilityneinum" class="shortinput" maxlength="1" min="1" max="9">件をまとめて見るリンクを表示</label></li>
		...
		qq|
			...
			<div class="fieldsubset">
				...
				<ul class="list">
					...
					<li><label>続きを読むラベル(<span class="important">共通用</span>)：<input type="text" value="$setdat{'readmorebtnlabel'}" name="readmorebtnlabel" class="semiinput"></label><span class="inputguide">(標準：続きを読む)</span></li>
					...
					<li><label>読後に畳むラベル：<input type="text" value="$setdat{'readmorecloselabel'}" name="readmorecloselabel" class="semiinput"></label><span class="inputguide">(標準：畳む)</span></li>
	...
	push(@ctrlset,([
		...
		'pPostUrl',
		'投稿本文の表示／URL処理',
		'uh,usage/#howtouse-makelink;ch,custom/#customizecss-link',
		'36em',
		qq|
			<p>
				▼URLが書かれた場合の表示：<br>
			...
			<ul class="list">
				<li><input type="checkbox" name="urlautolink" id="urlautolink" value="1" $setdat{'urlautolink'}><label for="urlautolink">URLを自動でリンクにする</label><span class="inputguide">(※1)</span>
					...
					<p class="innersubtitle withseparator">▼リンクラベルを指定したリンクの記法：</p>
					<ul class="list">
						<li><label><input type="checkbox" name="urllabeledlink" value="1" $setdat{'urllabeledlink'}>URLの直前に[ラベル]があればその文字列でリンクにする</label></li>
						<li><label><input type="checkbox" name="urlparenthesislink" value="1" $setdat{'urlparenthesislink'}>URLを丸括弧で囲む [ラベル](URL) 記法も使用可能にする</label></li>
					</ul>
					...
					<ul class="list">
						...
						<li><label><input type="checkbox" name="urlexpandapplemusic" value="1" $setdat{'urlexpandapplemusic'}>URLの直前に[AppleMusic]ラベルがあれば音楽を埋め込む</label></li>
						<li><label><input type="checkbox" name="urlexpandcodepen" value="1" $setdat{'urlexpandcodepen'}>URLの直前に[CodePen]ラベルがあればCodePenを埋め込む</label></li>
					</ul>
			...
			<ul>
				<li><input type="checkbox" name="cemojiallow" id="cemojiallow" value="1" $setdat{'cemojiallow'}><label for="cemojiallow">カスタム絵文字の表示を許可</label><span class="inputguide">(※1)</span>
					<ul class="subopt">
						...
						<li><input type="checkbox" name="cemojiccjs" id="cemojiccjs" value="1" $setdat{'cemojiccjs'}><label for="cemojiccjs">絵文字のクリックでそのコードをコピーする<span class="inputguide">(※4)</span></label>
		...
		qq|
			...
			<ul class="list">
				<li><input type="checkbox" name="hashtaglinkize" id="hashtaglinkize" value="1" $setdat{'hashtaglinkize'}><label for="hashtaglinkize">本文中のハッシュタグをリンクにする</label>
					<ul class="list">
						<li><label>ハッシュタグのリンクは、<input type="text" value="$setdat{'hashtagcut'}" name="hashtagcut" class="shortinput">文字目で切る</label><span class="inputguide">(標準：25／※1)</span></li>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li>
					<input type="checkbox" name="searchhighlight" id="searchhighlight" value="1" $setdat{'searchhighlight'}><label for="searchhighlight">検索結果に含まれる検索語を強調表示する</label>
					<ul class="list">
						<li>
							検索語の強調に使う要素：
							<select name="searchmarkup"><option value="0" $searchmarkup[0]>strong</option><option value="1" $searchmarkup[1]>span</option><option value="2" $searchmarkup[2]>mark</option><option value="3" $searchmarkup[3]>em</option></select><span class="inputguide">(標準：strong)</span>
						</li>
					</ul>
				</li>
			</ul>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul>
				<li><label><input type="checkbox" name="usesearchcmnd" value="1" $setdat{'usesearchcmnd'}>検索コマンドも使用可能にする</label><span class="inputguide">(※1,3)</span></li>
			</ul>
			<p class="withseparator">
				▼検索対象に追加する属性：<span class="inputguide">(※2,3)</span>
			</p>
			<ul>
				<li><label><input type="checkbox" name="catidinsearch" value="1" $setdat{'catidinsearch'}>カテゴリID <small>も検索対象にする</small></label></li>
				<li><label><input type="checkbox" name="catnameinsearch" value="1" $setdat{'catnameinsearch'}>カテゴリ名 <small>も検索対象にする</small></label></li>
				<li><label><input type="checkbox" name="dateinsearch" value="1" $setdat{'dateinsearch'}>投稿日時 <small>も検索対象にする</small></label></li>
				<li><label><input type="checkbox" name="useridinsearch" value="1" $setdat{'useridinsearch'}>ユーザID <small>も検索対象にする</small></label></li>
				<li><label><input type="checkbox" name="usernameinsearch" value="1" $setdat{'usernameinsearch'}>ユーザ名 <small>も検索対象にする</small></label></li>
				<li><label><input type="checkbox" name="postidinsearch" value="1" $setdat{'postidinsearch'}>投稿番号 <small>も検索対象にする</small></label></li>
				<li><label><input type="checkbox" name="poststatusinsearch" value="1" $setdat{'poststatusinsearch'}>投稿状態 <small>も検索対象にする</small></label><span class="inputguide">(※4)</span></li>
			</ul>
			<p class="inputguide">
				※1：検索対象を明示するコマンドを使って全文検索できるようにします。<br>
				※2：ここでOFFにした属性は、検索コマンドによる検索も無効になります。<br>
				※3：管理画面の投稿一覧画面での検索時には、ここの設定に関係なく全項目がON(有効)です。<br>
				※4：鍵付き(lock)、下げる(rear)等の状態を検索できるようにします。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><input type="checkbox" name="caladdweekrow" id="caladdweekrow" value="1" $setdat{'caladdweekrow'}><label for="caladdweekrow">カレンダーの先頭行に曜日名を表示</label>
					<ul class="list">
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><label>固定表示する投稿番号：<input type="text" value="$setdat{'fixedpostids'}" name="fixedpostids" class="stdlinput"></label><span class="inputguide">(複数指定可／※1)</span></li>
				<li><label>固定表示を示すラベル：<input type="text" value="$setdat{'fixedpostsign'}" name="fixedpostsign" class="stdlinput"></label><span class="inputguide">(標準：(先頭固定)／※2)</span></li>
				<li>先頭に固定する投稿の日付表示：
					<ul class="subopt">
						<li>
			...
			<p class="inputguide">
				※1：投稿番号を半角カンマ記号「,」で区切れば複数指定できます。上限なし。<br>（先頭に固定されるのは、表示対象が何も限定されていない状況でのみです。）<br>
				※2：内側スキンの中に [[NEW]] と書いた場所に表示されます。<br>（先頭固定されている投稿には「直近投稿を示すサイン」は表示されません。）<br>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><label>入力欄前のラベル：<input type="text" value="$setdat{'secretkeylabel'}" name="secretkeylabel" class="stdlinput"></label> <span class="inputguide">(※4)</span></li>
				<li><label>鍵プレースホルダ：<input type="text" value="$setdat{'secretkeyph'}" name="secretkeyph" class="stdlinput"></label> <span class="inputguide">(※5)</span></li>
				<li><label>送信ボタンラベル：<input type="text" value="$setdat{'secretkeybtn'}" name="secretkeybtn" class="stdlinput"></label> <span class="inputguide">(標準：投稿を見る)</span></li>
				<li><label>鍵違いエラー表示：<input type="text" value="$setdat{'secretkeyerror'}" name="secretkeyerror" class="stdlinput"></label> <span class="inputguide">(標準：鍵が違います。再度入力して下さい。)</span></li>
			...
			<ul class="list">
				<li><input type="checkbox" name="allowonelineinsecret" id="allowonelineinsecret" value="1" $setdat{'allowonelineinsecret'}><label for="allowonelineinsecret">本文の1行目だけは常に見せる（全1行の場合は除く）</label> <span class="inputguide">(※6)</span>
					<ul class="list">
						<li><input type="checkbox" name="allow2ndlineinsecret" id="allow2ndlineinsecret" value="1" $setdat{'allow2ndlineinsecret'}><label for="allow2ndlineinsecret">本文の2行目も常に見せる</label> <span class="inputguide">(※7)</span>
					</ul>
				</li>
				<li><label><input type="checkbox" name="allowonepictinsecret" value="1" $setdat{'allowonepictinsecret'}>n枚目の画像を [[ONEPICT:n]] 記法等で表示する</label> <span class="inputguide">(※8)</span></li>
			</ul>
			<p class="inputguide">
				...
				※7：1行目をタイトルにするスキンで、隠された内容を本文冒頭で説明したい場合などに。<br>
				※8：OGPでは1枚目の画像を見せたい場合や、ギャラリーモードやそれに準ずるスキン等で(鍵の入力前でも)指定番目の画像を見せたい場合にはここをONにして下さい。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				...
				※2：「表示条件が限定されていない状況」では常に非表示になり、「1投稿の単独表示時」では常に表示されます。それ以外の状況でどう表示するかを選択できます。<br><br>
	...
	push(@ctrlset,([
		...
		qq|
			<p>
				▼表示対象がない場合に表示されるシステムメッセージ <span class="inputguide">※HTML使用可</span>
			</p>
			<ul class="list">
			...
			<p class="withseparator">
				▼ハッシュタグリストが空の場合のシステムメッセージ <span class="inputguide">※$allowhtmlmsg。</span>
			</p>
			<ul class="list">
				<li><label><input type="text" value="$setdat{'msgnotags'}" name="msgnotags" class="longinput"></label><span class="inputguide">(標準：ハッシュタグは見つかりませんでした。(または、まだ集計されていません。))</span></li>
			</ul>
			<p class="withseparator">
				▼表示順序を示す用語：
			</p>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※英字1文字だけを指定できます。キーの組み合わせはブラウザによって異なりますが、Windows版のChromeやFirefoxでは [Alt]+[Shift]+[英字] で移動できます。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><label><span style="display:inline-block;min-width:9em;">管理画面内の入力欄</span>：<input type="text" value="$setdat{'textareasizenormal'}" name="textareasizenormal" class="shortinput">行</label> <span class="inputguide">(標準:12行／最低:4行／※1)</span></li>
				<li><label><span style="display:inline-block;min-width:9em;">QUICKPOST入力欄</span>：<input type="text" value="$setdat{'textareasizequick'}"  name="textareasizequick" class="shortinput">行</label> <span class="inputguide">(標準:4.3行／最低:1.8行／※1)</span></li>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※1：指定の日時に達するまでは下書き投稿のように扱われます。未来の日付でも、現実に存在しない日時の場合は予約投稿にはなりません。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※「ページの表示」設定で、画像の表示関連機能を無効化していれば、それぞれの機能に対応するボタンは設定に関係なく常に非表示になります。<br>
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※「ページの表示」設定で装飾機能を無効化していれば、設定に関係なく常に非表示になります。<br>
			</p>
			<p class="withseparator">
				▼表示する装飾ボタンの選択： <span class="inputguide">(※1)</span><br>
			</p>
			<table class="decBtnTable standard">
				<tr><th>新規/編集画面</th><th>QUICKPOST</th><th>ラベル</th><th>出力要素<span class="inputguide">(※2)</span></th></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnEonA" value="1" $setdat{'showDecoBtnEonA'}><span>強調	</span></label></td><td><label><input type="checkbox" name="showDecoBtnEonQ" value="1" $setdat{'showDecoBtnEonQ'}><span>強調	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelE'}"  name="decoBtnLabelE" class="shortinput"></td><td>$decoBtnElmnt{'E'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnBonA" value="1" $setdat{'showDecoBtnBonA'}><span>太字	</span></label></td><td><label><input type="checkbox" name="showDecoBtnBonQ" value="1" $setdat{'showDecoBtnBonQ'}><span>太字	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelB'}"  name="decoBtnLabelB" class="shortinput"></td><td>$decoBtnElmnt{'B'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnIonA" value="1" $setdat{'showDecoBtnIonA'}><span>斜体	</span></label></td><td><label><input type="checkbox" name="showDecoBtnIonQ" value="1" $setdat{'showDecoBtnIonQ'}><span>斜体	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelI'}"  name="decoBtnLabelI" class="shortinput"></td><td>$decoBtnElmnt{'I'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnUonA" value="1" $setdat{'showDecoBtnUonA'}><span>下線	</span></label></td><td><label><input type="checkbox" name="showDecoBtnUonQ" value="1" $setdat{'showDecoBtnUonQ'}><span>下線	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelU'}"  name="decoBtnLabelU" class="shortinput"></td><td>$decoBtnElmnt{'U'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnQonA" value="1" $setdat{'showDecoBtnQonA'}><span>引用	</span></label></td><td><label><input type="checkbox" name="showDecoBtnQonQ" value="1" $setdat{'showDecoBtnQonQ'}><span>引用	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelQ'}"  name="decoBtnLabelQ" class="shortinput"></td><td>$decoBtnElmnt{'Q'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnDonA" value="1" $setdat{'showDecoBtnDonA'}><span>取消線	</span></label></td><td><label><input type="checkbox" name="showDecoBtnDonQ" value="1" $setdat{'showDecoBtnDonQ'}><span>取消線	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelD'}"  name="decoBtnLabelD" class="shortinput"></td><td>$decoBtnElmnt{'D'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnSonA" value="1" $setdat{'showDecoBtnSonA'}><span>小さめ	</span></label></td><td><label><input type="checkbox" name="showDecoBtnSonQ" value="1" $setdat{'showDecoBtnSonQ'}><span>小さめ	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelS'}"  name="decoBtnLabelS" class="shortinput"></td><td>$decoBtnElmnt{'S'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnTonA" value="1" $setdat{'showDecoBtnTonA'}><span>極小	</span></label></td><td><label><input type="checkbox" name="showDecoBtnTonQ" value="1" $setdat{'showDecoBtnTonQ'}><span>極小	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelT'}"  name="decoBtnLabelT" class="shortinput"></td><td>$decoBtnElmnt{'T'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnRonA" value="1" $setdat{'showDecoBtnRonA'}><span>ルビ	</span></label></td><td><label><input type="checkbox" name="showDecoBtnRonQ" value="1" $setdat{'showDecoBtnRonQ'}><span>ルビ	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelR'}"  name="decoBtnLabelR" class="shortinput"></td><td><span class="inputguide">(専用の要素)</span></td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnLonA" value="1" $setdat{'showDecoBtnLonA'}><span>リスト	</span></label></td><td><label><input type="checkbox" name="showDecoBtnLonQ" value="1" $setdat{'showDecoBtnLonQ'}><span>リスト	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelL'}"  name="decoBtnLabelL" class="shortinput"></td><td><span class="inputguide">(専用の要素)</span></td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnConA" value="1" $setdat{'showDecoBtnConA'}><span>文字色	</span></label></td><td><label><input type="checkbox" name="showDecoBtnConQ" value="1" $setdat{'showDecoBtnConQ'}><span>文字色	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelC'}"  name="decoBtnLabelC" class="shortinput"></td><td>$decoBtnElmnt{'C'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnMonA" value="1" $setdat{'showDecoBtnMonA'}><span>背景色	</span></label></td><td><label><input type="checkbox" name="showDecoBtnMonQ" value="1" $setdat{'showDecoBtnMonQ'}><span>背景色	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelM'}"  name="decoBtnLabelM" class="shortinput"></td><td>$decoBtnElmnt{'M'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnHonA" value="1" $setdat{'showDecoBtnHonA'}><span>隠す	</span><span class="inputguide">(※3)</span></label></td><td><label><input type="checkbox" name="showDecoBtnHonQ" value="1" $setdat{'showDecoBtnHonQ'}><span>隠す	</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelH'}"  name="decoBtnLabelH" class="shortinput"></td><td>$decoBtnElmnt{'H'}</td></tr>
				<tr><td><label><input type="checkbox" name="showDecoBtnFonA" value="1" $setdat{'showDecoBtnFonA'}><span>自由装飾</span><span class="inputguide">(※4)</span></label></td><td><label><input type="checkbox" name="showDecoBtnFonQ" value="1" $setdat{'showDecoBtnFonQ'}><span>自由装飾</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelF'}"  name="decoBtnLabelF" class="shortinput"></td><td>$decoBtnElmnt{'F'}</td></tr>
			</table>
			<p class="inputguide">
				※1:ボタンが非表示でも、装飾記法を直接書けばすべて使えます。<br>
				※2:<span style="color:crimson;">HTMLの文法に詳しくない場合は、「<b>(注)</b>」表記のある要素を採用<b>しないで</b>下さい。</span>それらの要素を使うと、本文の書き方によっては他の装飾が正しく適用されなくなる(＝親要素の装飾が途中で打ち切られる)可能性があります。詳しくは<a href="$aif{'puburl'}usage/#howtouse-chardecoration-element-options" $mpconfirm>このヘルプ項目の「C」部分</a>をご参照下さい。<br>
				※3:隠す範囲の出力をどの要素にしても、その要素がinlineで描画されるかblockで描画されるかは、「ページの表示」設定の【投稿本文の表示／テキスト】内の「展開する範囲の表示方法」の設定値に依存しますのでご注意下さい。<br>
				※4:自由装飾には事前準備が必須です。<a href="$aif{'puburl'}usage/#howtouse-chardecoration-class" $mpconfirm>詳細はこちら</a>。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※「ページの表示」設定でURLの自動リンク機能を無効化していれば、常に非表示になります。
			</p>
			<p class="withseparator">
				▼表示するリンクボタンの選択： <span class="inputguide">(※1)</span><br>
			</p>
			<table class="decBtnTable standard">
				...
				<tr><td><label><input type="checkbox" name="showLinkBtnImg" value="1" $setdat{'showLinkBtnImg'}><span>画像埋込リンク</span><span class="inputguide">(※2)</span></label></td><td><input type="text" value="$setdat{'linkBtnImgLabel'}"  name="linkBtnImgLabel" size="12"></td></tr>
			...
			<p class="inputguide">
				※1:「任意URLリンク」以外の各ボタンは、それぞれの機能が無効に設定されている場合は上記の設定に関係なく非表示になります。<br><br>
				※2：この機能で画像を埋め込んでもギャラリーモードには表示されません。この項目は過去との互換性のために残してあるだけです。画像をURLで指定して表示するには、『画像』ボタン側の「任意画像を挿入」機能を使う方が便利です。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul>
				<li>
					ハッシュタグ前後に自動挿入する半角スペースの位置：
					<select name="hashtagspace"><option value="0" $hashtagspace[0]>自動挿入しない</option><option value="1" $hashtagspace[1]>ハッシュタグの前に挿入する</option><option value="2" $hashtagspace[2]>ハッシュタグの後に挿入する</option></select>
				</li>
			</ul>
			<p class="withseparator">
				▼リストに追加する項目：
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※1：装飾対象やリンク対象を事前に範囲選択して<strong>いない</strong>状態でも、「範囲選択して下さい」というエラーを<strong>出さずに</strong>各種記法を編集領域に挿入します。(画面幅が1024px以下の場合は、ここの設定に関係なく常に挿入できます。)<br>
	...
	push(@ctrlset,([
		...
		qq|
			<p>
				▼ギャラリーモードの基本設定： <span class="inputguide">(<a href="?mode=gallery" target="_blank">ギャラリーモードを見る</a>)</span>
			</p>
			<ul class="list">
				<li><input type="checkbox" name="galleryoutput" id="galleryoutput" value="1" $setdat{'galleryoutput'}><label for="galleryoutput">ギャラリー表示機能を使う</label> <span class="inputguide">(※1)</span>
					<ul class="list">
						<li><label>スキン格納ディレクトリ名：<input type="text" value="$setdat{'galleryskindir'}" name="galleryskindir" class="midinput"></label><span class="inputguide">(標準:skin-gallery／※2)</span></li>
	...
	push(@ctrlset,([
		...
		'soPicts',
		'画像一覧モードの出力',
		'ch,custom/#customizeinfo-pictsskin',
		'33.5em',
		qq|
			<p>
				▼画像一覧モードの基本設定： <span class="inputguide">(<a href="?mode=picts" target="_blank">画像一覧モードを見る</a>)</span>
			</p>
			<ul class="list">
				<li><input type="checkbox" name="pictsmodeoutput" id="pictsmodeoutput" value="1" $setdat{'pictsmodeoutput'}><label for="pictsmodeoutput">画像一覧表示機能を使う</label> <span class="inputguide">(※1)</span>
					<ul class="list">
						<li><label>スキン格納ディレクトリ名：<input type="text" value="$setdat{'pictsmodeskindir'}" name="pictsmodeskindir" class="midinput"></label><span class="inputguide">(標準:skin-picts／※2)</span></li>
						<li><label>画像一覧の名称：<input type="text" value="$setdat{'pictsmodename'}" name="pictsmodename"></label></li>
						<li><label><input type="checkbox" name="pictsmodedatebar" value="1" $setdat{'pictsmodedatebar'}>日付境界バーを挿入する</label> <span class="inputguide">(※3)</span></li>
						<li><label><input type="checkbox" name="pictsmodesituation" value="1" $setdat{'pictsmodesituation'}>該当件数とページ番号を表示する</label> <span class="inputguide">(※4)</span></li>
					</ul>
					<p class="innersubtitle withseparator">
						▼「一覧外」フラグ付き画像の扱い：
					</p>
					<ul class="subopt">
						<li>
							<label><input type="radio" name="pictsmodenolisted" value="0" $pictsmodenolisted[0]>一切表示しない</label><br>
							<label><input type="radio" name="pictsmodenolisted" value="1" $pictsmodenolisted[1]>「下げる」投稿と同じ形態として扱う</label> <span class="inputguide">(※5)</span><br>
						</li>
					</ul>
					<p class="innersubtitle withseparator">
					...
					<ul class="list">
						<li><label><input type="text" value="$setdat{'pictsmodeentries'}" name="pictsmodeentries" size="5">個</label></li>
					</ul>
				</li>
			</ul>
			<p class="inputguide">
				※1：画像保存用ディレクトリ内に存在するすべての画像を、専用スキンで一覧表示する機能です。(投稿本文で使われているかどうかに関係なく、アップロードされている画像は全部が対象になります。)<br>
				※2：絶対パスまたは相対パス(※CGIの位置から見た相対パス)でも指定可能。ここで指定したスキン以外でも、skinパラメータを使えば複数のスキンを併用できます。<br>
				※3：挿入条件や挿入内容は「ページの表示／全体」での設定が使われます。<br>
				※4：状況表示部分に件数を表示し、2ページ目以降ではページ番号も表示します。<br>
				※5：一覧時には表示されません。表示される状況は「ページの表示」タブ→【下げた投稿の表示】の設定に従います。<br>
				...
				<li><input type="checkbox" name="rssoutput" id="rssoutput" value="1" $setdat{'rssoutput'}><label for="rssoutput">RSSフィードを出力する</label>
					<ul class="subopt">
						<li>
							...
							<ul class="subopt">
								<li><label>スキン格納ディレクトリ名：<input type="text" value="$setdat{'rssskindir'}" name="rssskindir" class="midinput"></label><span class="inputguide">(標準:rss)</span></li>
							</ul>
						</li>
					</ul>
					<p class="innersubtitle withseparator">
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><input type="checkbox" name="ogpoutput" id="ogpoutput" value="1" $setdat{'ogpoutput'}><label for="ogpoutput">OGP＋Twitter Cardを出力する</label><span class="inputguide">(※1)</span>
					...
					<ul class="list selectboxset">
						...
						<li>og:image
							<ul class="subbox">
								<li><input type="checkbox" checked disabled><label>共通画像のURL：<input type="text" value="$setdat{'ogimagecommonurl'}" name="ogimagecommonurl" placeholder="https://"></label><span class="inputguide">(※5,<strong class="important highlight">A</strong>)</span></li>
								<li><input type="checkbox" name="ogimageuse1st" id="ogimageuse1st" value="1" $setdat{'ogimageuse1st'}><label for="ogimageuse1st">投稿に画像が含まれる場合は、1つ目の画像URLを指定<span class="inputguide">(※6,<strong class="important highlight">A</strong>)</span><br>（投稿に画像が含まれない場合は、共通画像URLを指定）</label>
									<ul class="subopt">
										<li><input type="checkbox" name="ogimagehidensfw" id="ogimagehidensfw" value="1" $setdat{'ogimagehidensfw'}><label for="ogimagehidensfw">その画像がNSFWフラグ付きなら代替画像のURLを使う</label>
											<ul class="subopt list">
												<li>代替画像のURL：<input type="text" value="$setdat{'ogimagensfwurl'}" name="ogimagensfwurl" placeholder="https://"></label><span class="inputguide">(※7,<strong class="important highlight">A</strong>)</span></li>
											</ul>
										</li>
									</ul>
								</li>
			...
			<p class="inputguide">
				※1:外側スキンの [[OGP]] の位置に出力されますが、スキンがHTMLとして成立していればその記述がなくても自動出力されます。出力条件の詳細は<a href="$aif{'puburl'}custom/#customizeinfo-autocomplement" $mpconfirm>このヘルプ</a>をご覧下さい。
				<span style="color:crimson;"><b class="highlight">※A：</b>ここで設定するOGP情報（特に画像URL）は、SNS側で一度でも表示を試した後はSNS側が一時保持するため、<strong class="highlight">設定を変更してもすぐには反映されません!!!</strong> 変更内容の確認は、出力ソースを見るか、もしくは数時間程度空けてから表示をお試し下さい。</span><br>
		...
		qq|
			...
			<ul class="list">
				<li><input type="checkbox" name="sitemappageoutput" id="sitemappageoutput" value="1" $setdat{'sitemappageoutput'}><label for="sitemappageoutput">サイトマップページ表示機能を使う</label> <span class="inputguide">(※1)</span>
					<ul class="list">
						<li><label>スキン格納ディレクトリ名：<input type="text" value="$setdat{'sitemappageskindir'}" name="sitemappageskindir" class="midinput"></label><span class="inputguide">(標準:skin-sitemap／※2)</span></li>
		...
		qq|
			...
			<ul class="list">
				<li><input type="checkbox" name="imagemaxlimits" id="imagemaxlimits" value="1" $setdat{'imagemaxlimits'}><label for="imagemaxlimits">投稿できる画像サイズや画像の保存容量に上限を設ける</label><br>
					...
					<ul class="list">
						<li><label>画像1枚あたりの最大サイズ：<input type="text" value="$setdat{'imagemaxbytes'}" name="imagemaxbytes" class="halfinput"><b class="unit unitKB">KB</b></label> <span class="inputguide">(標準：5120KB)</span></li>
						<li><label>保存可能な画像の最大枚数：<input type="text" value="$setdat{'imagefilelimit'}" name="imagefilelimit" class="halfinput">枚</label> <span class="inputguide">(標準：10000枚)</span></li>
						<li><label>画像保存に使える最大容量：<input type="text" value="$setdat{'imagestoragelimit'}" name="imagestoragelimit" class="halfinput"><b class="unit unitMB">MB</b></label> <span class="inputguide">(標準：300MB)</span></li>
					</ul>
				</li>
			</ul>
			<p class="inputguide">
	...
	push(@ctrlset,([
		...
		'29.5em',
		qq|
			<p class="inputguide" style="margin-top:1em;">
				※これらの設定群は、実験的な実装段階だったり、使うためには別途何らかの配慮が必要だったりする機能などです。事前にヘルプドキュメントをよくご参照の上でご活用下さい。
			</p>
			...
			<div id="highsets">
				...
				<ul>
					<li><label><input type="checkbox" name="loadeditcssjs" value="1" $setdat{'loadeditcssjs'}>編集画面で、edit.cssとedit.jsを(あれば)読み込む</label> <span class="inputguide">(※1)</span></li>
					<li><label><input type="checkbox" name="loadedithtm" value="1" $setdat{'loadedithtm'}>スキンDIRに edit.htmがあれば、編集画面で読み込む</label> <span class="inputguide">(※2)</span></li>
					<li><label><input type="checkbox" name="outputsitemapxml" value="1" $setdat{'outputsitemapxml'}>SITEMAP XMLを sitemap.txt ファイルに静的出力する</label> <span class="inputguide">(※3)</span></li>
					<li><label><input type="checkbox" name="outputindexhtml" value="1" $setdat{'outputindexhtml'}>HOMEページを index.html ファイルにも静的出力する</label> <span class="inputguide">(※4)</span></li>
					<li><label><input type="checkbox" name="allowhtmlondesc" value="1" $setdat{'allowhtmlondesc'}>カテゴリ概要文の入力欄にHTMLの記述を許可する</label></li>
					...
					<li>連携コード：<input type="text" value="$setdat{'coopcode'}" name="coopcode" placeholder=""> <span class="inputguide">(※5)</span></li>
				</ul>
				<script>document.getElementById('highsets').style.display='none';</script>
				<p class="inputguide">
					※1：適用スキンに関係なく、CGI本体と同じディレクトリにあるファイルのみが読み込まれます。→<a href="$aif{'puburl'}custom/#ctrl-posteditpage" $mpconfirm>ヘルプ</a><br>
					※2：そのとき適用されているスキンの格納ディレクトリにあるファイルが読み込まれます。→<a href="$aif{'puburl'}custom/#ctrl-inshtml-posteditpage" $mpconfirm>ヘルプ</a><br>
					※3：新規投稿のタイミングで静的ファイルとして書き出します。→<a href="$aif{'puburl'}usage/#sitemapxmlstatic" $mpconfirm>ヘルプ</a><br>
					※4：ログイン済みのユーザがトップページ(＝何の表示条件も限定されていない状況の1ページ目)にアクセスしたタイミング(＝例えば投稿直後)で、そのページの内容をindex.htmlファイルにも出力します。→<a href="$aif{'puburl'}usage/#howtouse-outputindexhtml" $mpconfirm>ヘルプ</a><br>
					※5：何らかの公式サービス等で、個人情報を登録することなく所有権を確認するために使われる予定ですが、今のところ何にも使っていません。^^;
				</p>
				<p class="withseparator">
					▼動作形態：<br>
				</p>
				<ul>
					<li>
						<label><input type="radio" name="operationtype" value="0" $operationtype[0]>一般公開(無限定表示)</label> <span class="inputguide">(※6)</span><br>
						<label><input type="radio" name="operationtype" value="1" $operationtype[1]>ログイン者限定表示</label> <span class="inputguide">(※7)</span><br>
						<ul class="list">
							<li><label><input type="checkbox" name="guestidtype" value="1" $setdat{'guestidtype'}>ゲストIDによる投稿を禁止する</label> <span class="inputguide">(※8)</span></li>
							<li>初期画面に表示するメッセージ：<input type="text" value="$setdat{'privoploginmsg'}" name="privoploginmsg" placeholder="" class="stdlinput"> <span class="inputguide">(※9)</span></li>
						</ul>
					</li>
				</ul>
				<p class="inputguide">
					※6：誰でも閲覧できる通常の動作形態です。(デフォルト)<br>
					※7：ログインしていない限りどのページも表示されなくなります。ただし、事前の環境設定が必須です。詳しくは<a href="$aif{'puburl'}usage/#howtouse-privateop" $mpconfirm>ヘルプ</a>をご覧下さい。この機能を使っても<strong class="important">画像は誰でも閲覧可能なまま</strong>な点にご注意下さい。画像は保護されません。<br>
					※8：ユーザ権限が「ゲスト(Lv.1)」に設定されたIDを、『読むだけ』のIDとして使えるようにします。(既存の投稿には影響しません。)<br>
					※9：標準のメッセージに戻したい場合は、空欄にして下さい。<br>
				</p>
			</div>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※てがろぐCGIを同一ドメイン下に複数個設置する場合は、それぞれの設定画面で<strong class="important">異なる識別文字列</strong>を設定して下さい。すると、それぞれのCGIで個別にログイン状態を維持できるようになります。<br>
				※この機能をOFFにしたり、同じ文字列を指定したりすると、異なる「てがろぐ」にアクセスする度に自動ログアウトしてしまいます。（※同じ文字列を指定しても、IDを共用できるわけでは<strong>ありません</strong>）
			</p>
		|
	]));
...
# 	]));
	...
	foreach my $oc (@ctrlset) {
		...
		$tab1html .= &fcts::makefieldset(@$oc[1],@$oc[2],&fcts::makefieldhelpbtns(@$oc[3]),@$oc[4],@$oc[5]);	# 設定項目の中身を生成
		push(@tab1index,'<a href="#' . @$oc[1] . '">' . @$oc[2] . '</a>');										# ショートカットリンク
    }
    $tab1html = &fcts::makeshortcuslinkbox(@tab1index) . $tab1html;

	my $tab2html = '';
	my @tab2index = ();
	foreach my $oc (@ctrlset) {
		if( @$oc[0] ne 'tab2' ) { next; }
		$tab2html .= &fcts::makefieldset(@$oc[1],@$oc[2],&fcts::makefieldhelpbtns(@$oc[3]),@$oc[4],@$oc[5]);	# 設定項目の中身を生成
		push(@tab2index,'<a href="#' . @$oc[1] . '">' . @$oc[2] . '</a>');										# ショートカットリンク
    }
    $tab2html = &fcts::makeshortcuslinkbox(@tab2index) . $tab2html;

	my $tab3html = '';
	my @tab3index = ();
	foreach my $oc (@ctrlset) {
		if( @$oc[0] ne 'tab3' ) { next; }
		$tab3html .= &fcts::makefieldset(@$oc[1],@$oc[2],&fcts::makefieldhelpbtns(@$oc[3]),@$oc[4],@$oc[5]);	# 設定項目の中身を生成
		push(@tab3index,'<a href="#' . @$oc[1] . '">' . @$oc[2] . '</a>');										# ショートカットリンク
    }
    $tab3html = &fcts::makeshortcuslinkbox(@tab3index) . $tab3html;

	my $tab5html = '';
	my @tab5index = ();
	foreach my $oc (@ctrlset) {
		if( @$oc[0] ne 'tab5' ) { next; }
		$tab5html .= &fcts::makefieldset(@$oc[1],@$oc[2],&fcts::makefieldhelpbtns(@$oc[3]),@$oc[4],@$oc[5]);	# 設定項目の中身を生成
		push(@tab5index,'<a href="#' . @$oc[1] . '">' . @$oc[2] . '</a>');										# ショートカットリンク
    }
    $tab5html = &fcts::makeshortcuslinkbox(@tab5index) . $tab5html;

	my $tab4html = '';
	my @tab4index = ();
	foreach my $oc (@ctrlset) {
		if( @$oc[0] ne 'tab4' ) { next; }
		$tab4html .= &fcts::makefieldset(@$oc[1],@$oc[2],&fcts::makefieldhelpbtns(@$oc[3]),@$oc[4],@$oc[5]);	# 設定項目の中身を生成
		push(@tab4index,'<a href="#' . @$oc[1] . '">' . @$oc[2] . '</a>');										# ショートカットリンク
    }
    $tab4html = &fcts::makeshortcuslinkbox(@tab4index) . $tab4html;
...
$tab4html
	...
	my $css = '<style>
		...
		.stdlinput { width:11em; }	.midinput { width:9em; }
		.semiinput { width:6em; }	.halfinput { width:4.5em; }
		.shortinput { width:3em; }	.onechar { width: 1.5em; }
...
sub adminTrychangeset
{
	...
	my %trySetdat = (
		...
		utilityneighbor		=> $cgi->param('utilityneighbor'	) || 0 ,
		utilityneinum		=> $cgi->param('utilityneinum'		) || 1 ,	# 0は許容できない値
		...
		pictsmodeoutput		=> $cgi->param('pictsmodeoutput'	)	 || 0 ,
		pictsmodename		=> $cgi->param('pictsmodename'		)	 || '' ,
		pictsmodeskindir	=> $cgi->param('pictsmodeskindir'	)	 || 'skin-picts' ,
		pictsmodedatebar	=> $cgi->param('pictsmodedatebar'	)	 || 0 ,
		pictsmodesituation	=> $cgi->param('pictsmodesituation'	)	 || 0 ,
		pictsmodeentries	=> $cgi->param('pictsmodeentries'	)	 || 1 ,
		pictsmodenolisted	=> $cgi->param('pictsmodenolisted'	)	 || 0 ,
		...
		msgnotags			=> $cgi->param('msgnotags'			) || '' ,	# (空白可)
		...
		allow2ndlineinsecret	=> $cgi->param('allow2ndlineinsecret') || 0 ,
		...
		urllabeledlink		=> $cgi->param('urllabeledlink'		) || 0 ,
		urlparenthesislink	=> $cgi->param('urlparenthesislink'	) || 0 ,
		...
		urlexpandcodepen	=> $cgi->param('urlexpandcodepen'		) || 0 ,
		...
		hashtagspace		=> $cgi->param('hashtagspace'		) || 0 ,
		...
		searchmarkup		=> $cgi->param('searchmarkup'		) || 0 ,
		...
		showDecoBtnBonA	=> $cgi->param('showDecoBtnBonA') || 0 ,	showDecoBtnBonQ	=> $cgi->param('showDecoBtnBonQ') || 0 ,	decoBtnLabelB	=> $cgi->param('decoBtnLabelB') || 'Ｂ' ,	decoBtnElmntB	=> $cgi->param('decoBtnElmntB') || 'b',		,
		showDecoBtnConA	=> $cgi->param('showDecoBtnConA') || 0 ,	showDecoBtnConQ	=> $cgi->param('showDecoBtnConQ') || 0 ,	decoBtnLabelC	=> $cgi->param('decoBtnLabelC') || '色' ,	decoBtnElmntC	=> $cgi->param('decoBtnElmntC') || 'span',	,
		showDecoBtnDonA	=> $cgi->param('showDecoBtnDonA') || 0 ,	showDecoBtnDonQ	=> $cgi->param('showDecoBtnDonQ') || 0 ,	decoBtnLabelD	=> $cgi->param('decoBtnLabelD') || '消' ,	decoBtnElmntD	=> $cgi->param('decoBtnElmntD') || 'del',	,
		showDecoBtnEonA	=> $cgi->param('showDecoBtnEonA') || 0 ,	showDecoBtnEonQ	=> $cgi->param('showDecoBtnEonQ') || 0 ,	decoBtnLabelE	=> $cgi->param('decoBtnLabelE') || '強' ,	decoBtnElmntE	=> $cgi->param('decoBtnElmntE') || 'em',	,
		showDecoBtnFonA	=> $cgi->param('showDecoBtnFonA') || 0 ,	showDecoBtnFonQ	=> $cgi->param('showDecoBtnFonQ') || 0 ,	decoBtnLabelF	=> $cgi->param('decoBtnLabelF') || '○' ,	decoBtnElmntF	=> $cgi->param('decoBtnElmntF') || 'span',	,
		showDecoBtnHonA	=> $cgi->param('showDecoBtnHonA') || 0 ,	showDecoBtnHonQ	=> $cgi->param('showDecoBtnHonQ') || 0 ,	decoBtnLabelH	=> $cgi->param('decoBtnLabelH') || '隠す' ,	decoBtnElmntH	=> $cgi->param('decoBtnElmntH') || 'span',	,
		showDecoBtnIonA	=> $cgi->param('showDecoBtnIonA') || 0 ,	showDecoBtnIonQ	=> $cgi->param('showDecoBtnIonQ') || 0 ,	decoBtnLabelI	=> $cgi->param('decoBtnLabelI') || 'Ｉ' ,	decoBtnElmntI	=> $cgi->param('decoBtnElmntI') || 'i',		,
		...
		showDecoBtnMonA	=> $cgi->param('showDecoBtnMonA') || 0 ,	showDecoBtnMonQ	=> $cgi->param('showDecoBtnMonQ') || 0 ,	decoBtnLabelM	=> $cgi->param('decoBtnLabelM') || '背' ,	decoBtnElmntM	=> $cgi->param('decoBtnElmntM') || 'span',	,
		showDecoBtnQonA	=> $cgi->param('showDecoBtnQonA') || 0 ,	showDecoBtnQonQ	=> $cgi->param('showDecoBtnQonQ') || 0 ,	decoBtnLabelQ	=> $cgi->param('decoBtnLabelQ') || '”' ,	decoBtnElmntQ	=> $cgi->param('decoBtnElmntQ') || 'q',		,
		...
		showDecoBtnSonA	=> $cgi->param('showDecoBtnSonA') || 0 ,	showDecoBtnSonQ	=> $cgi->param('showDecoBtnSonQ') || 0 ,	decoBtnLabelS	=> $cgi->param('decoBtnLabelS') || '小' ,	decoBtnElmntS	=> $cgi->param('decoBtnElmntS') || 'small',	,
		showDecoBtnTonA	=> $cgi->param('showDecoBtnTonA') || 0 ,	showDecoBtnTonQ	=> $cgi->param('showDecoBtnTonQ') || 0 ,	decoBtnLabelT	=> $cgi->param('decoBtnLabelT') || '極' ,	decoBtnElmntT	=> $cgi->param('decoBtnElmntT') || 'small',	,
		showDecoBtnUonA	=> $cgi->param('showDecoBtnUonA') || 0 ,	showDecoBtnUonQ	=> $cgi->param('showDecoBtnUonQ') || 0 ,	decoBtnLabelU	=> $cgi->param('decoBtnLabelU') || 'Ｕ' ,	decoBtnElmntU	=> $cgi->param('decoBtnElmntU') || 'u',		,
		...
		operationtype		=> $cgi->param('operationtype'		) || 0 ,
		guestidtype			=> $cgi->param('guestidtype'		) || 0 ,
		privoploginmsg		=> $cgi->param('privoploginmsg'		) || '' ,
		...
		outputsitemapxml	=> $cgi->param('outputsitemapxml'	) || 0 ,
		outputindexhtml		=> $cgi->param('outputindexhtml'	) || 0 ,
		loadeditcssjs		=> $cgi->param('loadeditcssjs'		) || 0 ,
		loadedithtm			=> $cgi->param('loadedithtm'		) || 0 ,
		allowhtmlondesc		=> $cgi->param('allowhtmlondesc'	) || 0 ,
	...
	# ログイン者限定表示指定時の環境判断
	# --------------------------------
	my $privateOpDenied = 0;
	if( $trySetdat{'operationtype'} == 1 ) {
		# ログイン者限定表示が指定されている場合、.htaccessファイルの存在を確認する
		if(! -f '.htaccess' ) {
			# .htaccessファイルがなければ
			if( $setdat{'operationtype'} != 1 ) {
				# 今はログイン者限定表示ではない場合 (＝今回の設定でログイン者限定表示に設定された場合)
				# 設定値を拒否
				$privateOpDenied = 1;
				$trySetdat{'operationtype'} = 0;
			}
			else {
				# 既にログイン者限定表示だった場合
				# 設定値は維持、注意喚起だけ表示
	...
	# 新着リストの再生成：掲載個数等を変更している可能性があるため
	my $latestlisthtml = &makelatestlist( $trySetdat{'latestlistup'} , $trySetdat{'latestlistparts'} , $trySetdat{'latesttitlecut'} );
	$trySetdat{'latestlisthtml'} = $latestlisthtml;

	# フリースペースの改行を変換（※データは1行にする必要がある）
	$trySetdat{'freespace'} =~ s/\r\n/<br>/g;
	$trySetdat{'freespace'} =~ s/\r|\n/<br>/g;		# 改行がCR+LFではない場合も

	# EXTRACSSの保存前処理
	$trySetdat{'extracss'} =~ s/^(\s|\t|\r|\n)+$//;	# 先頭から終わりまでに、空白とタブと改行しか存在しないなら全部消す。
	$trySetdat{'extracss'} =~ s/\r\n/<br>/g;		# 改行を変換（※データは1行にする必要がある）
	$trySetdat{'extracss'} =~ s/\r|\n/<br>/g;		# 改行がCR+LFではない場合も
	...
	if( $flagDemo{'RefuseToChangeSettings'} != 1 ) {
		...
		# ----------------------
		my $nodirs = '';
		my $dxfn = &fcts::forsafety($bmsdata);
		...
		# ログイン者限定表示の注意喚起
		if( $privateOpDenied == 1 ) {
			# 今回の設定変更でログイン者限定表示にされたが、.htaccessファイルがない場合
			$nodirs .= qq|<li><strong class="important" style="background-color:#fee;">【ログイン者限定表示の設定を<b style="background-color:red;color:yellow;border-radius:5px;padding:0 5px">拒否</b>しました】</strong><br>&#10145;「ログイン者限定表示」に切り替える操作をされましたが、その設定は自動で<strong>キャンセルされました</strong>。CGI本体と同じディレクトリに <b><a href="$aif{'puburl'}setup/#htaccessfile">.htaccess ファイル</a></b>が見つからないためです。もし、データファイル $dxfn へのアクセスをブロックしていない場合は、データファイルを直接閲覧(ダウンロード)された場合に<strong>本文がすべて読めてしまう</strong>ため、ログイン者限定表示の意味がありません。まずは、<a href="$aif{'puburl'}setup/#htaccessfile">この.htaccess ファイル</a>を設置して下さい。もし、より浅いディレクトリの.htaccessやその他の手段で適切にアクセス制御がなされている場合は、CGI本体と同じディレクトリに.htaccessファイルはなくても良いのですが、そこまで自動判別はできないため一律で拒否する仕様になっています。（※この文章の意味が分からない場合は、「ログイン者限定表示」の使用は避ける方が無難です。サーバ側のアクセス制限機能であるBasic認証等をお使い下さい。）詳しくは<a href="$aif{'puburl'}usage/#howtouse-privateop">ヘルプ</a>をご覧下さい。</li>|;
		}
		elsif( $privateOpDenied == 2 ) {
			# 既にログイン者限定表示だったが、今.htaccessファイルがない場合
			$nodirs .= qq|<li>現在、既に「ログイン者限定表示」に設定された状態で稼働中ですが、CGI本体と同じディレクトリに<a href="$aif{'puburl'}setup/#htaccessfile">.htaccess ファイル</a>が見つかりません。もし、データファイル $dxfn へのアクセスをブロックできていない場合は、データファイルを直接閲覧(ダウンロード)された場合に本文がすべて読めてしまうため、<strong>ログイン者限定表示の意味がありません</strong>のでご注意下さい。</li>|;
		}

		# 注意喚起・警告等の案内が存在するなら出力用に整形
		if( $nodirs ne '' ) {
...
sub adminChangepass
{
		...
		<form action="$cgipath" method="post"><input type="hidden" name="idwork" value="$work">
			<fieldset>
				...
				<ul class="inputs">
					...
					<li><label><span class="itemhead">アイコン：</span> <input type="text" value="$uii" name="tryicon" id="tryicon" placeholder="https://"></label> <span class="notice">※アイコン画像のURLなど。(省略可／Base64可)</span><div id="tiAlert" style="display:none;">▲URLが https:// ではなく http:// で始まっていると、環境によっては表示されない点にご注意下さい。</div></li>
	...
	|;
	# アイコンのURLが http:// で始まっている場合に警告するためのJavaScript：
	$msg .= &fcts::tooneline( qq|
		<script>
			function checkInput() {
				/* アイコン入力欄の値を取得 */
				var inputValue = document.getElementById('tryicon').value;
				var tiAlert = document.getElementById('tiAlert');
				if((location.protocol === 'https:') && (inputValue.startsWith('http://'))) {
					/* 自身がHTTPSで接続されている場合で、値が「http://」で始まっているなら警告を表示 */
					tiAlert.style.display = 'block';
				}
				else {
					/* そうでなければ警告を消す */
					tiAlert.style.display = 'none';
				}
			}
			checkInput();
			document.getElementById('tryicon').addEventListener('input', checkInput);
		</script>
	| );
	my $css = '<style>
		...
		input:focus:placeholder-shown { color: #ccc; }
		.sendui { font-size:1.2em; }
		#tiAlert { font-size:0.7em; color:crimson; background-color:#ff8; }
	</style>';
...
{
	my $optflags = shift @_ || 'UDHCOM';	# 指定がなければデフォルト順序を使う
	...
	foreach my $flag (split //, $optflags) {
		...
		elsif( $flag =~ m/M|G|g|P|p|S|s/ )	{
			# 表示モード指定なら
			my $mcret = getModeCheckbox($flag);
			if( $mcret ne '' ) {
				# 出力内容がある場合だけli要素を出力
				$htmlform .= qq|\n<li class="somode">| . getModeCheckbox($flag) . q|</li>|;
			}
		}
...
sub getModeCheckbox
{
	...
	if( $gors eq 'gallery' ) {
		my $galleryname = &fcts::forsafety($setdat{'galleryname'});
		if( $galleryname eq '' ) { $galleryname = 'ギャラリー'; }	# 名称が空文字だったら、とりあえずギャラリーと表記しておく。
		$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="gallery" name="mode"$checked><span>$galleryname内に限定して検索</span></label>|;
	}
	elsif( $gors eq 'picts' ) {
		my $pictsmodename = &fcts::forsafety($setdat{'pictsmodename'});
		if( $pictsmodename eq '' ) { $pictsmodename = '画像一覧'; }	# 名称が空文字だったら、とりあえず画像一覧と表記しておく。
		$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="picts" name="mode"$checked><span>$pictsmodename内に限定して検索</span></label>|;
	}
	elsif( $gors eq 'sitemap' ) {
		my $sitemappageyname = &fcts::forsafety($setdat{'sitemappageyname'});
		if( $sitemappageyname eq '' ) { $sitemappageyname = 'サイトマップ'; }	# 名称が空文字だったら、とりあえずサイトマップと表記しておく。
		$searchoptions .= qq|<label class="searchoption"><input type="checkbox" value="sitemap" name="mode"$checked><span>検索結果を$sitemappageynameとして表示</span></label>|;
	}
...
sub adminSkinlist
{
	...
	foreach my $oneskin (@skins) {
		...
		elsif( $oneskin eq $setdat{'pictsmodeskindir'} ) {
			# 画像一覧モード用に指定されているディレクトリ名と一致したら
			$skinlinks .= qq|<tr><th>$oneskin</th><td colspan="2"><small class="otherpurpose">※<a href="?mode=picts">画像一覧モード</a>用に指定されています。</small></td></tr>\n|;
		}
	...
	# 任意のディレクトリを指定するフォーム（とガイドメッセージ）
	my $skininotherdir = '';
	my $skininotherguide = '';
	if( $setdat{'allowupperskin'} == 1 ) {
		# CGI本体の位置より浅いディレクトリを参照する相対パスや、絶対パスで指定されたスキンを使用可能にする場合のみ
		$skininotherdir .= qq|<div class="otherskinbox">
			<script>
			...
			<form action="$cgipath" method="post" class="osdForm">
				...
				<p class="osdNote">※本番適用する前に、プレビューで正常動作を確認して下さい。<br>※存在しないディレクトリや、スキンが格納されていないディレクトリを指定して本番適用すると、てがろぐの表示がエラー画面になります。その場合は、管理画面から再度このページにアクセスし、指定を解除するか他のスキンを指定して下さい。</p>
			</form>
			</div>\n|;
		# ガイド表示
		$skininotherguide = '(孫ディレクトリ以降の階層は走査しませんが、手動で指定すれば使うことはできます。)';
	}
	else {
		# 相対パスまたは絶対パスが使えない場合のガイド表示
		$skininotherguide = '<br>※もし、相対パスまたは絶対パスを使って任意の場所にあるスキンを適用したい場合は、事前に [設定]→[システム設定]の【スキンの適用制限】で、その使用を許可しておく必要があります。(許可してから再度このページを表示すると、任意のスキンを適用するための専用入力欄が表示されます。)';
	}
		...
		if( $rentalflag == 1 ) {
			...
			$msg .= q|
					<ul class="sysguide">
						...
						<li><strong class="important">簡易</strong>：下記の「本番適用(簡易)」ボタンを押すことでも本番適用できます。ただし、スキンHTML内に相対パスの記述があると正しく表示できない可能性があります。</li>
					</ul>
					<small>※いま既に簡易適用しているスキンを本則適用に移す場合は、そうする前にまず、この画面でスキンの適用設定をデフォルト状態に戻して下さい。</small>
				</li>
			|;
		}
		$msg .= '</ul>' . $skinlinks;
	...
	if( $rentalflag == 1 ) {
		...
		# 通常モードなら
		$msg .= q|<p class="noticebox">※この画面には、CGI設置ディレクトリ内の『サブディレクトリに格納された別スキンファイル』が一覧表示されます。<br>※CGI本体のあるディレクトリに「1スキン＝1サブディレクトリ」でアップロードすると、ここに一覧で現れます。| . $skininotherguide . q|<br>※プレビューや本番適用(簡易)では、スキンHTMLの記述方法によっては、CSSが正しく適用できない場合があります。<br>※CGIの設定でスキンファイル名を変更している場合は、その変更後のスキンファイル名で存在しているスキンのみを認識します。</p>|;
	}
...
sub adminApplySkin
{
	...
	# ありがちな指定ミスを防ぐ
	if( $newskin =~ m/^[A-Za-z]:/ ) {
		# 英字1文字＋コロン記号で始まっている場合 (アップロード欄だと誤解？)
		&showadminpage('NOT AVAILABLE','','<p>ローカルにあるスキンを直接指定することはできません。</p><p>使いたいスキンは、Webサーバ上の「CGIのあるディレクトリ」にアップロードして下さい。</p>','CSA','');
		exit;
	}
	elsif( $newskin =~ m|://| ) {
		# 英字1文字＋コロン記号で始まっている場合 (アップロード欄だと誤解？)
		&showadminpage('NOT AVAILABLE','','<p>スキンをURLで指定することはできません。</p><p>相対パスまたは絶対パスを使って、Webサーバ上でのスキンの存在位置を指定して下さい。</p>','CSA','');
		exit;
	}
	...
	if( $flagDemo{'RefuseToChangeSettings'} != 1 ) {
		...
		if( $newskin ne '' ) {
			...
			$msg .= q|
				<p>下記の注意事項もご一読下さい。</p>
				<p class="noticebox">
					※<strong>もし正常に閲覧できない場合は：</strong><br>この<strong class="important">簡易</strong>本番適用機能では、スキンHTMLの書き方によっては（相対パスの記述を自動変換できずに）正しく表示できない場合があります。その場合は、スキンの本番適用を一旦解除した後に、スキンの構成ファイルを「CGIと同じディレクトリ」に手動で移動させて、デフォルトスキンとしてご使用下さい。（その際は、スキンの位置を移動させるよりも前に、まずは簡易本番適用を解除して下さい。）
				</p>
			|;
		}
		else {
...
sub makepostform
{
	...
	# ▽テキストエリア出力
	my $textarea = qq|<p class="line-textarea"><!-- この投稿欄のheightやplaceholderは管理画面の[設定]→[投稿欄の表示]で設定可能 --><textarea class="tegalogpost" name="comment" id="tegalogpostUNIQUERAND" style="$textareaheight" placeholder="$placeholder" $scriptctrlenter accesskey="| . &fcts::forsafety($setdat{'postareakey'}) . '">' . &fcts::forsafety($comment) . '</textarea></p>';
		...
		<script>
			...
			function insLinkUNIQUERAND(kind){
				...
				if( kind == 'U' ) {
					...
					if( !uis.match(/^https?:\\/\\//) ) {
						if( uis.match(/^(\\.{0,2}\\/\|mailto:.+\|tel:.+)/) ) { uis = '(' + uis + ')'; }
						else { alert('http(s)://から始まるURLか、./ や ../ で始まる相対パスか、 / で始まる絶対パスか、mailto:やtel:で始まる文字列以外は指定できません。同階層のファイルを指定する際はまず先頭に「./」を付けて下さい。'); tArea.focus(); return; }
					}
					else {
						uis += ' ';
					}
					txtSel = txtSel.replace(/\\\[/g, '&#91;') .replace(/\\\]/g, '&#93;');	/* 角括弧をエスケープする */
					uis = uis.replace(/\\\|/g, '%7C');	/* 縦棒をエスケープする */
					insText = '[' + txtSel + ']' + uis;
				}
				else if( kind == 'N' ) {
					$gtgtnote
					...
					if( uisCheckUNIQUERAND(uis,tArea) == 0 ) { return; }
					if( !uis.match(/twitter.com\|x.com/)) { alert('TwitterのURLではなさそうです。\\ntwitter.comまたはx.comドメインのURLを入力して下さい。'); return; }
	...
	if( $setdat{'showHashtagBtnStyle'} != 2 ) {
		...
		foreach my $oh (@hashtagdata) {
			...
			# ハッシュタグの前後に空白を入れる設定の場合には加える
			my $spaceBefore = '';
			my $spaceAfter = '';
			if( $setdat{'hashtagspace'} == 1 ) { $spaceBefore = ' '; }		# ハッシュタグの前に空白を入れる場合
			elsif( $setdat{'hashtagspace'} == 2 ) { $spaceAfter = ' '; }	# ハッシュタグの後に空白を入れる場合(デフォルト)
			$htagopts .= qq|<option value="$spaceBefore#$ohn[0]$spaceAfter">#$ohn[0] ($ohn[1])</option>|;
...
# 			$categoryinputset .= q|<span class="helpbutton" onclick="alert('カテゴリの追加や編集は、管理画面の「カテゴリ管理」でできます。');">？</span>|;
	...
	# 中身カラでの投稿操作を抑止するスクリプト
	my $noemptypostSet = '';
	my $noemptypostJS = '';
	if( $isNPOST ) {
		# ノーマル投稿フォームの場合だけ
		$noemptypostSet = q| onsubmit="return emptyPostCheck();"|;
		$noemptypostJS = &fcts::tooneline( q|
<script>
	function emptyPostCheck() {
		var textarea = document.getElementById('tegalogpostUNIQUERAND');
		if(textarea.value === '') {
			alert('投稿本文を何か入力して下さい。');
			return false;
		}
		return true;
	}
	...
	# ログイン者限定表示の場合にはその旨を表示 (QUICKPOSTのみ)
	my $inPrivateSign = '';
	if(( $setdat{'operationtype'} == 1 ) && $isQUICK ) {
		$inPrivateSign = "\n\t\t\t\t" . q|<small class="inPrivate"><a href="#" style="text-decoration:none;" onclick="alert('このCGIは現在、ログインしていない限りページが表示されない内輪形態で稼働中です。この状態では、一般のアクセス者はページを閲覧できませんのでご注意下さい。解除するには、[設定]→[システム設定]の【基本動作】項目で「一般公開(無限定表示)」に切り替えて下さい。'); return false;">【&#128274;ログイン者限定表示中】</a></small>|;
	}
	...
	my $cgipath = &getCgiPath();
	my $postformhtml = qq|
		<!-- 投稿フォーム(UNIQUERAND) -->$noemptypostJS
		<form action="$cgipath" method="post" class="postform" enctype="multipart/form-data"$noemptypostSet>
			$idForm $dateForm
			$textarea
			<div class="line-control">
				...
				<!-- 機能ボタン -->$funcinputset $inPrivateSign
			</div>
...
sub putpoststohead
{
	...
	@gotnums = &fcts::alltrim(@gotnums);	# すべてをtrimする
	my @topnums = ();
...
# ---------------------------------------------- #
# データ全体を走査して年月リストを生成＆カウント #		引数： 臨時かどうか(1=臨時:キャッシュしない／0=通常更新:キャッシュする)
# ---------------------------------------------- #
sub datadatecounter
{
	my $tempuse  = shift @_ || 0;
...
sub makeHashtaglistParts
{
	...
	if( $#hashtagdata == -1 ) {
		...
		if( $setdat{'hashtagcup'} == 1 ) {
			# ハッシュタグが集計される設定なら
			$hashtagliststring  .= '<li class="notexist">' . $setdat{'msgnotags'} . '</li>';
		}
		else {
			# ハッシュタグが集計されない設定なら
			$hashtagliststring  .= q|<li class="notexist">ハッシュタグの集計機能は無効に設定されています。</li>|;
		}
		$hashtagpullstring3 .= q|<option class="head">(なし)</option>|;
	}
	$hashtagliststring  .= "\n</ul>\n";
	$hashtagpullstring3 .= "\n</select>";
...
# --------------------------------
# モードに応じたリンクに書き換える		引数1：モードを示す文字列（注：必ずしも正しいモード名が渡されるとは限らない）、引数2：対象のHTML
# --------------------------------		返値：書き換えたHTML
sub linkadjustbymode
{
	...
	# モード名を小文字に変換して、モード別に加える文字列を用意する
	my $modestr = '';
	if( $mode =~ /^(GALLERY|PICTS|SITEMAP|RSS)$/ ) {
		# 指定されたモード名が GALLERY、PICTS、SITEMAP、RSS のどれかだった場合のみ
		$modestr = lc($mode);
	}
	elsif( $mode eq 'KEEPMODE' ) {
		# 今のモードを維持する場合
		$modestr = &fcts::forsafety( $cp{'mode'} );
	}
	else {
		# 引数がおかしい場合
...
# ---------------------- #
# 別スキンの適用(上書き) #	※仕様：管理画面にアクセスしようとしている状況では、(エラーがあるかどうかに関係なく)何の処理もせずに返す。
# ---------------------- #	引数1：スキンのあるディレクトリのパス、引数2：エラー時に表示するモード用文字列
sub overrideskins
{
	# Adminモードでは実際にスキンを適用する必要はないので、Adminモードの場合は適用処理を省略する
	if( $cp{'mode'} eq 'admin' ) {
		return;
	}
	...
	if( -d $tryskindir ) {
		...
		if( &isSkinUseMode() ) {
			...
			if(( $cp{'skindir'} eq '' ) && ( $cp{'mode'} eq 'view' )) {
				# 簡易本番適用の場合(＝パラメータではスキンが指定されていないのにこの関数が実行されていて、動作モードが view の場合)
...
sub passfront
{
	...
	if( &fcts::checkpass() == 2 ) {
		...
		my $passreset = '';
		if( $loginform =~ /disabled placeholder="\(入力不要\)"/ ) {
			my $psif = &fcts::forsafety($passfile);
			if( &fcts::sessioncount() == 0 ) {
				# セッション数もゼロならリセットされたものと解釈する
				$passreset = '<div class="resetguide"><p class="resettitle">パスワードをリセットしましたか？</p><div class="resetnote"><p>パスワード・セッションID格納ファイル（' . $psif . 'ファイル）の中身が空にされたか、または初期ファイルで上書きされたため、全ユーザのログイン状態が破棄され、全ユーザのパスワードが無効化されました。</p><p><b>【意図した結果の場合】</b>➡下記のログインフォームで無条件ログインができますので、ログイン後にパスワードを再設定して下さい。</p><p><b>【意図していない場合】</b>➡おそらく、誤って' . $psif . 'ファイルを上書きアップロードしてしまったのでしょう。このファイルは（パスワードを初期化したい場合を除いて）上書きしてはいけません。既に全ユーザのパスワードは無効化されていますので、下記のログインフォームからログインした後に、パスワードを再設定して下さい。全てのユーザのパスワードが「なし」状態にリセットされている点にご注意下さい。現状のままで放置するのは危険ですから、必ず全ユーザのパスワードを再設定して下さい。（※てがろぐを' . qq|<a href="$aif{'puburl'}setup/#howtovup">| . 'バージョンアップさせる際</a>に、このファイルを上書きする必要は<strong class="important">ありません</strong>。）</p></div></div>';
			}
		}
		$errmsg = '<p style="font-weight:bold;">ようこそ、「てがろぐ - Fumy Otegaru Memo Logger -」へ！</p>' . $passreset . '<p>現在、パスワードが1つも設定されていないため、誰でもログインできる状態になっています。<br>下記のボタンをクリックしてログインして下さい。</p>';
...
{
	# ------------------------------------
	# ▼Step.1：現状の設定を新たに読み込む		※引数に含まれていない項目は現状維持。その際、一時的に変更されている値を保存してしまうのを防ぐために、まずは「ファイルに保存されている設定値」を得る。
	# ------------------------------------
	my %newdat = ();

	&fcts::safetyfilename($setfile,1);	# 不正ファイル名の確認
	open(SETIN, $setfile) or &errormsg("savesettings:(書き換えのための事前読み込み)<br>設定ファイルが開けませんでした。");
	flock(SETIN, 1);
	foreach my $setline (<SETIN>) {
		if( $setline =~ m/^(.+?)=(.*)$/ ) { $newdat{$1} = $2; }	# 記録されている項目名(=の左側)に該当する連想配列(%newdat)に、各値(=の右側)を代入する。
	}
	close SETIN;
	...
	# ----------------------------------
	# バージョン番号・文字コード・出力日付
	$newdat{'[version]'} = $versionnum;
	$newdat{'{charset}'} = $charcode;
	$newdat{'{savedate}'} = &fcts::getdatetimestring();

	# A:保存用の連想配列%newdatの内容を、引数の配列に指定されている内容に限って書き換える
	# B:現設定を保持している連想配列%setdatの内容も、同様に書き換えておく (※保存処理後に継続する処理で新設定値を利用する可能性があるため)
	my $renewcount = 0;
	foreach my $tryoneset (@_) {
		if( $tryoneset =~ m/^(.+?)=(.*)$/ ) {
			# A:引数に指定された[キー＆値]のセットを、連想配列%newdatへ上書きして、上書き個数をカウントする。
			$newdat{$1} = $2;
			$renewcount++;

			# B:同じ内容を連想配列%setdatへも上書き。
			$setdat{$1} = $2;
		}
	}
	...
	if( $renewcount > 0 ) {
		...
		# 連想配列%newdatの中身をキーでソートした上で出力形式に整形して書き出す
		for my $name (sort keys %newdat) {
			print SETOUT "$name=$newdat{$name}\n";
		}
...
sub showadminpage
{
	...
	foreach my $flag (split //, $fflags) {
		...
		elsif( $flag eq 'N' )	{ $footer .= '<a href="' . &makeQueryString('mode=edit')					. '">さらに新規投稿</a>'; }
	...
	# ログイン者限定表示の場合にステータス表示を追加
	if( $setdat{'operationtype'} == 1 ) {
		$status .= q|<a href="#" class="btnlink statusbtn inprivate" onclick="alert('このCGIは現在、ログインしていない限りページが表示されない内輪形態で稼働中です。この状態では、一般のアクセス者はページを閲覧できませんのでご注意下さい。解除するには、[設定]→[システム設定]の【基本動作】項目で「一般公開(無限定表示)」に切り替えて下さい。'); return false;">&#128274;ログイン者限定表示中</a>|;
	}
...
sub infoboxmsg
{
	...
	<body>
		...
		<p id="link">$linkstr</p>
		<p id="foot">Powered by <a href="https://www.nishishi.com/cgi/tegalog/">てがろぐ</a></p>
	</body>
	</html>
EOM
	exit;
